...
//' @noRd
 ...
 arma::imat resolve_overlap_cpp(
     ...
     int              k_rep = 10,
     bool             singleton_as_block = false
 ) {
   int p = (int)adj_mat.n_cols;
   int n = (int)adj_mat.n_rows;
   int n_blocks = (int)blocks.n_rows;

   if (n_blocks < 1) return blocks;

   // --------------------------------------------------------------------------
   // Internal cleanup:
   //   1. Remove invalid intervals.
   //   2. Remove singleton intervals when singleton_as_block = FALSE.
   //   3. Remove contained intervals, including singleton blocks already
   //      represented inside larger blocks.
   // --------------------------------------------------------------------------
   auto clean_blocks = [&](arma::imat B) -> arma::imat {
     ...
     for (int i = 0; i < nb; i++) {
       ...
       if (!singleton_as_block && s == e) continue;

       valid.push_back(i);
     }

     if (valid.empty()) {
       return arma::imat(0, 2);
     }

     arma::imat C(valid.size(), 2);
     ...
     std::vector<int> ord(nc);
     std::iota(ord.begin(), ord.end(), 0);

     std::sort(ord.begin(), ord.end(), [&](int a, int b) {
       if (C(a, 0) != C(b, 0)) return C(a, 0) < C(b, 0);
       return C(a, 1) > C(b, 1);
     });
     ...
     for (int i = 0; i < nc; i++) {
       ...
       for (int j = 0; j < nc; j++) {
         ...
         if (contained && !same_interval) {
           keep[j] = false;
         }
       }
     }

     int nk = std::count(keep.begin(), keep.end(), true);
     arma::imat out(nk, 2);
   ...
   auto get_z = [&](int col0) -> const arma::vec& {
     ...
     if (sd_vec[col0] < 0.0) {
       ...
       if (sd > 1e-10) {
         ...
         z_vec[col0] = arma::zeros<arma::vec>(n);
       }
     }
