...
# Hand-built, fully deterministic fixture (rather than the ldx_* package data,
# whose exact allele calls aren't known ahead of time) so every expected value
# below is computed by hand, not just checked for shape.
#
# One block, "blk1", 3 SNPs (CHR "1", positions 1000/2000/3000):
#   SNP1 REF=A ALT=G (het -> IUPAC "R")
#   SNP2 REF=C ALT=T (het -> IUPAC "Y")
#   SNP3 REF=G ALT=C (het -> IUPAC "S")
# 7 individuals' dosage strings: "012" x3, "000" x2, "222" x1, ".00" x1 (missing).
#   "012" -> pos1 REF(A), pos2 het(Y), pos3 ALT(C) -> "AYC"
#   "000" -> all REF -> "ACG"
#   "222" -> all ALT -> "GTC"
#   ".00" -> missing marker at pos1 ("N"), then REF/REF -> "NCG" (only reachable
#            when missing_string doesn't match "." literally, see below)
.mk_decode_fixture <- function() {
  haps <- list(blk1 = c("012","012","012","000","000","222",".00"))
  attr(haps, "block_info") <- data.frame(
    block_id = "blk1", CHR = "1", start_bp = 1000L, end_bp = 3000L,
    n_snps = 3L, phased = FALSE, stringsAsFactors = FALSE
  )
  snp_info <- data.frame(
    SNP = c("rs1", "rs2", "rs3"), CHR = "1", POS = c(1000L, 2000L, 3000L),
    REF = c("A", "C", "G"), ALT = c("G", "T", "C"), stringsAsFactors = FALSE
  )
  list(haps = haps, snp_info = snp_info)
}

test_that("decode_haplotype_strings: returns all documented columns", {
  fx  <- .mk_decode_fixture()
  out <- decode_haplotype_strings(fx$haps, fx$snp_info)
  expect_true(all(c("block_id","CHR","start_bp","end_bp","hap_rank","hap_id",
                    "dosage_string","nucleotide_sequence","frequency",
                    "n_carriers","snp_positions","snp_alleles") %in% names(out)))
})

test_that("decode_haplotype_strings: default call decodes exact nucleotide strings, frequency, and n_carriers", {
  fx  <- .mk_decode_fixture()
  out <- decode_haplotype_strings(fx$haps, fx$snp_info)
  expect_equal(nrow(out), 3L)   # "012", "000", "222" -- ".00" excluded (missing)

  r1 <- out[out$dosage_string == "012", ]
  expect_equal(r1$nucleotide_sequence, "AYC")
  expect_equal(r1$hap_rank, 1L)
  expect_equal(r1$hap_id, "blk1_hap1")
  expect_equal(r1$frequency, round(3/6, 4))
  expect_equal(r1$n_carriers, 3L)

  r2 <- out[out$dosage_string == "000", ]
  expect_equal(r2$nucleotide_sequence, "ACG")
  expect_equal(r2$hap_rank, 2L)
  expect_equal(r2$frequency, round(2/6, 4))
  expect_equal(r2$n_carriers, 2L)

  r3 <- out[out$dosage_string == "222", ]
  expect_equal(r3$nucleotide_sequence, "GTC")
  expect_equal(r3$hap_rank, 3L)
  expect_equal(r3$frequency, round(1/6, 4))
  expect_equal(r3$n_carriers, 1L)
})

test_that("decode_haplotype_strings: snp_positions and snp_alleles are correct", {
  fx  <- .mk_decode_fixture()
  out <- decode_haplotype_strings(fx$haps, fx$snp_info)
  expect_true(all(out$snp_positions == "1:1000;1:2000;1:3000"))
  expect_true(all(out$snp_alleles   == "A/G;C/T;G/C"))
  expect_true(all(out$CHR == "1"))
  expect_true(all(out$start_bp == 1000L))
  expect_true(all(out$end_bp   == 3000L))
})

test_that("decode_haplotype_strings: min_freq filters low-frequency haplotypes", {
  fx  <- .mk_decode_fixture()
  out_default <- decode_haplotype_strings(fx$haps, fx$snp_info)                    # min_freq = 0.01
  out_strict  <- decode_haplotype_strings(fx$haps, fx$snp_info, min_freq = 0.2)    # drops the 0.1667 allele
  expect_equal(nrow(out_default), 3L)
  expect_equal(nrow(out_strict), 2L)
  expect_false("222" %in% out_strict$dosage_string)
  expect_true(all(c("012","000") %in% out_strict$dosage_string))
})

test_that("decode_haplotype_strings: top_n limits alleles per block", {
  fx  <- .mk_decode_fixture()
  out <- decode_haplotype_strings(fx$haps, fx$snp_info, top_n = 1L)
  expect_equal(nrow(out), 1L)
  expect_equal(out$dosage_string, "012")
  expect_equal(out$hap_rank, 1L)
})

test_that("decode_haplotype_strings: custom missing_string changes which individuals are excluded", {
  fx  <- .mk_decode_fixture()
  # "N" doesn't appear literally in any dosage string (including ".00"), so
  # nothing is treated as missing and ".00" becomes its own haplotype allele.
  out <- decode_haplotype_strings(fx$haps, fx$snp_info, missing_string = "N")
  expect_equal(nrow(out), 4L)
  expect_true(".00" %in% out$dosage_string)
  dot_row <- out[out$dosage_string == ".00", ]
  expect_equal(dot_row$nucleotide_sequence, "NCG")
  expect_equal(dot_row$n_carriers, 1L)
})

test_that("decode_haplotype_strings: errors without a block_info attribute", {
  haps <- list(blk1 = c("012","000"))   # no block_info attribute set
  snp_info <- .mk_decode_fixture()$snp_info
  expect_error(decode_haplotype_strings(haps, snp_info),
              "block_info attribute")
})

test_that("decode_haplotype_strings: errors when snp_info is missing required columns", {
  fx <- .mk_decode_fixture()
  bad_snp_info <- fx$snp_info[, c("SNP", "CHR", "POS")]   # drop REF, ALT
  expect_error(decode_haplotype_strings(fx$haps, bad_snp_info),
              "CHR, POS, REF, ALT")
})

test_that("decode_haplotype_strings: a block absent from block_info is silently skipped", {
  fx <- .mk_decode_fixture()
  fx$haps$blk2 <- c("00", "11")   # blk2 has no matching row in block_info
  out <- decode_haplotype_strings(fx$haps, fx$snp_info)
  expect_false("blk2" %in% out$block_id)
  expect_equal(nrow(out), 3L)     # blk1's rows are unaffected
})

test_that("decode_haplotype_strings: a block with no overlapping SNPs contributes zero rows", {
  fx <- .mk_decode_fixture()
  # Add a second block whose bp range covers no SNP in snp_info.
  fx$haps$blk2 <- c("00", "00")
  bi2 <- rbind(
    attr(fx$haps, "block_info")[, c("block_id","CHR","start_bp","end_bp","n_snps","phased")],
    data.frame(block_id = "blk2", CHR = "1", start_bp = 9000L, end_bp = 9500L,
              n_snps = 2L, phased = FALSE, stringsAsFactors = FALSE)
  )
  attr(fx$haps, "block_info") <- bi2
  out <- decode_haplotype_strings(fx$haps, fx$snp_info)
  expect_false("blk2" %in% out$block_id)
  expect_equal(nrow(out), 3L)
})

test_that("decode_haplotype_strings: a block where every individual is missing contributes nothing", {
  # Every dosage string contains the missing marker, so the whole block is
  # skipped internally (length(gam) == 0 -> next). With only one block in the
  # input, no data frame ever gets appended to the accumulator, so rbind()
  # is called on a list containing only its pre-allocated NULL slot and
  # returns NULL -- not an empty data.frame(). That distinction (NULL vs.
  # data.frame() with 0 rows) only matters if the caller does something like
  # nrow(out) without a NULL check first, which is exactly why it's worth
  # pinning down here.
  haps <- list(blk1 = c(".00", ".11", ".22"))
  attr(haps, "block_info") <- data.frame(
    block_id = "blk1", CHR = "1", start_bp = 1000L, end_bp = 3000L,
    n_snps = 3L, phased = FALSE, stringsAsFactors = FALSE
  )
  snp_info <- .mk_decode_fixture()$snp_info
  out <- decode_haplotype_strings(haps, snp_info)
  expect_null(out)
})

test_that("decode_haplotype_strings: an empty haplotypes list returns an empty data frame", {
  haps <- list()
  attr(haps, "block_info") <- data.frame(
    block_id = character(0), CHR = character(0),
    start_bp = integer(0), end_bp = integer(0),
    n_snps = integer(0), phased = logical(0), stringsAsFactors = FALSE
  )
  snp_info <- .mk_decode_fixture()$snp_info
  out <- decode_haplotype_strings(haps, snp_info)
