...
test_that("test_block_haplotypes: returns HapBlockR_haplotype_assoc", {
...
test_that("test_block_haplotypes: print method shows sig_metric", {
  ...
  expect_true(any(grepl("HapBlockR Haplotype Association", out)))
...
test_that("estimate_diplotype_effects: returns HapBlockR_diplotype", {
  skip_if_not_installed("rrBLUP")
  res <- estimate_diplotype_effects(.haps, blues = .blues_vec,
                                    blocks = ldx_blocks, verbose = FALSE)
  expect_s3_class(res, "HapBlockR_diplotype")
  expect_true(is.list(res))
})
...
test_that("estimate_diplotype_effects: print method works", {
  ...
  res <- estimate_diplotype_effects(.haps, blues = .blues_vec,
                                    blocks = ldx_blocks, verbose = FALSE)
  expect_output(print(res), "HapBlockR Diplotype Effect Results")
})
...
# -- normalize_method = "zscore" (previously untested) -----------------------
#
# score_favorable_haplotypes() has two normalize_method options when
# normalize = TRUE: "minmax" (default, rescales to [0,1]) and "zscore"
# (standardises to the panel's own mean/sd of raw block sums:
# index_i = (S_i - mean(S)) / sd(S)). Only "minmax" had test coverage.
# These tests hand-verify the zscore formula exactly against the raw
# (normalize = FALSE) stacking_index values from the same call.

test_that("score_favorable_haplotypes: normalize_method='zscore' matches the exact (S - mean(S))/sd(S) formula", {
  res_raw <- score_favorable_haplotypes(.haps, allele_effects = .ae,
                                        normalize = FALSE)
  res_z   <- score_favorable_haplotypes(.haps, allele_effects = .ae,
                                        normalize = TRUE,
                                        normalize_method = "zscore")
  # raw_S here is res_raw$stacking_index, which the function has already
  # rounded to 6 decimals (normalize = FALSE just returns round(block_sums,
  # 6)). Computing mean/sd from THIS rounded input and rounding the result
  # again compounds two roundings, whereas the function itself computes
  # mean/sd from the true unrounded block sums and rounds only once at the
  # end -- so this test's recomputed values can differ from the function's
  # own output by up to ~1e-6 in the last decimal purely from that double-
  # rounding, not from any formula mismatch. A looser tolerance absorbs that
  # while still tightly checking the (S - mean(S))/sd(S) relationship.
  raw_S <- res_raw$stacking_index[match(res_z$id, res_raw$id)]
  expected_z <- (raw_S - mean(raw_S)) / stats::sd(raw_S)
  expect_equal(res_z$stacking_index, expected_z, tolerance = 1e-4)
})

test_that("score_favorable_haplotypes: normalize_method='zscore' output has ~mean 0 and ~sd 1", {
  res_z <- score_favorable_haplotypes(.haps, allele_effects = .ae,
                                      normalize = TRUE,
                                      normalize_method = "zscore")
  expect_equal(mean(res_z$stacking_index), 0, tolerance = 1e-6)
  expect_equal(stats::sd(res_z$stacking_index), 1, tolerance = 1e-6)
})

test_that("score_favorable_haplotypes: normalize_method='zscore' preserves the same ranking as 'minmax' and raw", {
  # zscore, minmax, and raw are all monotonic (increasing, affine) transforms
  # of the same underlying raw block sums, so rank must be identical
  # regardless of normalize_method.
  res_raw    <- score_favorable_haplotypes(.haps, allele_effects = .ae,
                                           normalize = FALSE)
  res_minmax <- score_favorable_haplotypes(.haps, allele_effects = .ae,
                                           normalize = TRUE,
                                           normalize_method = "minmax")
  res_z      <- score_favorable_haplotypes(.haps, allele_effects = .ae,
                                           normalize = TRUE,
                                           normalize_method = "zscore")
  ord <- match(res_raw$id, res_z$id)
  expect_equal(res_z$rank[ord], res_raw$rank)
  ord2 <- match(res_minmax$id, res_z$id)
  expect_equal(res_z$rank[ord2], res_minmax$rank)
})

test_that("score_favorable_haplotypes: normalize_method defaults to 'minmax' when unspecified", {
  res_default <- score_favorable_haplotypes(.haps, allele_effects = .ae,
                                            normalize = TRUE)
  res_minmax  <- score_favorable_haplotypes(.haps, allele_effects = .ae,
                                            normalize = TRUE,
                                            normalize_method = "minmax")
  expect_equal(res_default$stacking_index, res_minmax$stacking_index)
})

test_that("score_favorable_haplotypes: errors on an invalid normalize_method", {
  expect_error(
    score_favorable_haplotypes(.haps, allele_effects = .ae,
...
test_that("compare_block_effects: returns HapBlockR_effect_concordance", {
  skip_if_not_installed("rrBLUP")
  ...
  expect_s3_class(conc, "HapBlockR_effect_concordance")
...
test_that("compare_block_effects: error on non-HapBlockR_haplotype_assoc input", {
  expect_error(
    compare_block_effects(list(allele_tests = data.frame()), list()),
    "HapBlockR_haplotype_assoc"
  )
})
...
test_that("compare_gwas_effects: raw GWAS path returns HapBlockR_effect_concordance", {
...
test_that(".match_blocks_by_position: finds matches for nudged boundaries", {
  res <- HapBlockR:::.match_blocks_by_position(blocks_A_pm, blocks_B_pm,
                                               overlap_min = 0.50)
  expect_s3_class(res, "data.frame")
...
test_that(".match_blocks_by_position: match_type is 'exact' when IDs agree", {
  ...
  res <- HapBlockR:::.match_blocks_by_position(ldx_blocks, ldx_blocks,
                                               overlap_min = 0.50)
...
test_that(".match_blocks_by_position: pop1_only when no overlap", {
  ...
  res <- HapBlockR:::.match_blocks_by_position(ldx_blocks[1:3, ], fake_b2,
                                               overlap_min = 0.50)
