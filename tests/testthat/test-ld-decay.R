...
test_that("compute_ld_decay: accepts NULL r2_threshold (no threshold)", {
  ...
  expect_s3_class(decay, "HapBlockR_decay")
})
...
test_that("compute_ld_decay: returns HapBlockR_decay object with required fields", {
  decay <- compute_ld_decay(ldx_geno, ldx_snp_info,
                            sampling     = "random",
                            r2_threshold = 0.1,
                            fit_model    = "loess",
...
test_that("compute_ld_decay: accepts HapBlockR_backend (matrix type)", {
...
test_that("print.HapBlockR_decay: prints without error", {
  decay <- compute_ld_decay(ldx_geno, ldx_snp_info,
                            r2_threshold = 0.1, n_pairs = 500L,
                            fit_model = "loess", verbose = FALSE)
  expect_output(print(decay), "HapBlockR LD Decay Analysis")
})
...
test_that("plot_ld_decay: errors on non-HapBlockR_decay input", {
  skip_if_not_installed("ggplot2")
  expect_error(plot_ld_decay(list(a = 1)), "HapBlockR_decay")
})
...
test_that("define_qtl_regions: accepts HapBlockR_decay object in ld_decay", {
  decay <- compute_ld_decay(ldx_geno, ldx_snp_info,
                            r2_threshold = 0.1, n_pairs = 1000L,
                            fit_model = "loess", verbose = FALSE)
