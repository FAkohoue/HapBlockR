...
# ==============================================================================
# include_dominance (additive + dominance dual-kernel GBLUP via BGLR)
# ==============================================================================
#
# Structural tests only (skip_if_not_installed("BGLR")), consistent with this
# package's existing convention for the bayesb/bayesc/bayesa marker_effect_method
# paths -- BGLR is a stochastic MCMC solver, so its exact output isn't
# hand-verifiable, only its shape/wiring. The include_dominance = FALSE
# (default) path is additive-only rrBLUP and unaffected by any of this, so it
# is checked here too as an explicit non-regression guard.

test_that("include_dominance = FALSE (default): unaffected, all new fields NULL", {
  res <- run_haplotype_prediction(ldx_geno, ldx_snp_info, ldx_blocks,
                                  blues = blues_vec, verbose = FALSE)
  expect_false(res$include_dominance)
  expect_null(res$dominance_deviation)
  expect_null(res$total_genetic_value)
  expect_null(res$G_dominance)
  expect_equal(res$solver_used, "rrBLUP")
})

test_that("include_dominance = TRUE errors when marker_effect_method != 'gblup'", {
  expect_error(
    run_haplotype_prediction(ldx_geno, ldx_snp_info, ldx_blocks,
                             blues = blues_vec, verbose = FALSE,
                             include_dominance = TRUE,
                             marker_effect_method = "rrblup"),
    "gblup"
  )
})

test_that("include_dominance must be a single TRUE/FALSE", {
  expect_error(
    run_haplotype_prediction(ldx_geno, ldx_snp_info, ldx_blocks,
                             blues = blues_vec, verbose = FALSE,
                             include_dominance = c(TRUE, FALSE)),
    "include_dominance"
  )
  expect_error(
    run_haplotype_prediction(ldx_geno, ldx_snp_info, ldx_blocks,
                             blues = blues_vec, verbose = FALSE,
                             include_dominance = "yes"),
    "include_dominance"
  )
})

test_that("include_dominance = TRUE: runs and returns the documented additional fields", {
  skip_if_not_installed("BGLR")
  res <- run_haplotype_prediction(ldx_geno, ldx_snp_info, ldx_blocks,
                                  blues = blues_vec, verbose = FALSE,
                                  include_dominance = TRUE,
                                  n_iter = 300L, burn_in = 100L, seed = 1L)
  expect_true(res$include_dominance)
  expect_equal(res$solver_used, "BGLR (dual-kernel RKHS: additive + dominance)")
  expect_type(res$dominance_deviation, "double")
  expect_type(res$total_genetic_value, "double")
  expect_false(is.null(names(res$dominance_deviation)))
  expect_equal(length(res$dominance_deviation), length(res$gebv))
  expect_true(is.matrix(res$G_dominance))
  expect_equal(dim(res$G_dominance), dim(res$G))
})

test_that("include_dominance = TRUE: total_genetic_value = gebv + dominance_deviation exactly", {
  skip_if_not_installed("BGLR")
  res <- run_haplotype_prediction(ldx_geno, ldx_snp_info, ldx_blocks,
                                  blues = blues_vec, verbose = FALSE,
                                  include_dominance = TRUE,
                                  n_iter = 300L, burn_in = 100L, seed = 1L)
  expect_equal(res$total_genetic_value,
              res$gebv + res$dominance_deviation[names(res$gebv)],
              tolerance = 1e-8)
})

test_that("include_dominance = TRUE: G_dominance is symmetric and dimnamed by individual ID", {
  skip_if_not_installed("BGLR")
  res <- run_haplotype_prediction(ldx_geno, ldx_snp_info, ldx_blocks,
                                  blues = blues_vec, verbose = FALSE,
                                  include_dominance = TRUE,
                                  n_iter = 300L, burn_in = 100L, seed = 1L)
  expect_equal(res$G_dominance, t(res$G_dominance), tolerance = 1e-8)
  expect_equal(rownames(res$G_dominance), rownames(ldx_geno))
})

test_that("include_dominance = TRUE: snp_effects/local_gebv/block_importance stay additive-only (unchanged shape)", {
  skip_if_not_installed("BGLR")
  res_a  <- run_haplotype_prediction(ldx_geno, ldx_snp_info, ldx_blocks,
                                     blues = blues_vec, verbose = FALSE)
  res_ad <- run_haplotype_prediction(ldx_geno, ldx_snp_info, ldx_blocks,
                                     blues = blues_vec, verbose = FALSE,
                                     include_dominance = TRUE,
                                     n_iter = 300L, burn_in = 100L, seed = 1L)
  expect_equal(length(res_ad$snp_effects), length(res_a$snp_effects))
  expect_equal(dim(res_ad$local_gebv), dim(res_a$local_gebv))
  expect_equal(names(res_ad$block_importance), names(res_a$block_importance))
})
