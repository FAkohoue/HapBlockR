...
test_that("cv_haplotype_prediction: returns HapBlockR_cv object", {
  ...
  expect_s3_class(cv, "HapBlockR_cv")
})
...
# -- Regression test: phased "gamete1|gamete2" haplotype strings -------------
#
# Prior to the fix in R/haplotype_analysis.R (see the long comment above
# `is_phased <- ...` in decompose_block_effects()), a phased pair string like
# "10110|01100" was treated as a single allele label. Its raw character
# length (2 * n_snps_in_block + 1, for the "|" separator) never matched
# n_snps_in_block, so the per-allele dosage-parsing check
# `if (n_pos != length(snp_order)) return(NA_real_)` fired for every single
# allele, and the block was silently dropped -- decompose_block_effects()
# returned an EMPTY data frame for any panel built from phased input, with no
# warning or error. The fix splits each "gamete1|gamete2" pair into its two
# constituent gamete alleles before frequency tabulation. This test proves
# the fix: on a phased haplotype panel, the function must return a
# non-empty result with correctly bounded frequencies and finite effects.

.si_ph  <- make_snpinfo(p = 20, chr = "1")
.blk_ph <- make_blocks(.si_ph, n_blocks = 4L)
.phased_ext <- make_phased(n = 40, p = 20, seed = 11L)  # hap1/hap2, 20 SNPs x 40 ind
.haps_ph_ext <- extract_haplotypes(.phased_ext, .si_ph, .blk_ph, min_snps = 3L)
.snpfx_ph <- setNames(rnorm(20, 0, 0.5), .si_ph$SNP)

test_that("decompose_block_effects: phased blocks are reported as phased in block_info", {
  bi <- attr(.haps_ph_ext, "block_info")
  expect_true("phased" %in% names(bi))
  expect_true(any(isTRUE(bi$phased) | bi$phased %in% TRUE))
})

test_that("decompose_block_effects: returns non-empty results on phased input (regression test for the gamete-splitting fix)", {
  res <- decompose_block_effects(
    .haps_ph_ext, .si_ph, .blk_ph, .snpfx_ph, min_freq = 0.02
  )
  # Before the fix this was always an empty (0-row) data frame -- the whole
  # point of this test is that it must NOT be empty.
  expect_s3_class(res, "data.frame")
  expect_gt(nrow(res), 0L)
})

test_that("decompose_block_effects: phased allele frequencies are valid proportions", {
  res <- decompose_block_effects(
    .haps_ph_ext, .si_ph, .blk_ph, .snpfx_ph, min_freq = 0.02
  )
  expect_true(all(res$frequency > 0 & res$frequency <= 1))
})

test_that("decompose_block_effects: phased allele_effect values are finite", {
  res <- decompose_block_effects(
    .haps_ph_ext, .si_ph, .blk_ph, .snpfx_ph, min_freq = 0.02
  )
  expect_true(all(is.finite(res$allele_effect)))
})

test_that("decompose_block_effects: phased alleles are single-gamete-length dosage strings, not gamete1|gamete2 pairs", {
  res <- decompose_block_effects(
    .haps_ph_ext, .si_ph, .blk_ph, .snpfx_ph, min_freq = 0.02
  )
  # A leftover "|" would mean the pair-splitting step was skipped/broken.
  expect_false(any(grepl("|", res$allele, fixed = TRUE)))
  # Each allele's character length must equal its block's SNP count (the
  # exact condition that silently failed pre-fix).
  for (bn in unique(res$block_id)) {
    sub <- res[res$block_id == bn, , drop = FALSE]
    expect_true(all(nchar(sub$allele) == sub$n_snps_block))
  }
})
