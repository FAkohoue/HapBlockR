...
test_that(".pairwise_interaction_scan: output structure on minimal matrix", {
  ...
  y <- rnorm(20)
  result <- HapBlockR:::.pairwise_interaction_scan(X, y, col_names = colnames(X))
...
test_that(".pairwise_interaction_scan: returns empty df for 1 column", {
  ...
  y <- rnorm(20)
  result <- HapBlockR:::.pairwise_interaction_scan(X, y)
  expect_s3_class(result, "data.frame")
...
test_that(".pairwise_interaction_scan: detects known interaction effect", {
  ...
  result <- HapBlockR:::.pairwise_interaction_scan(X, y, col_names = c("snp_a","snp_b"))
...
test_that("scan_block_epistasis: returns empty result when no sig blocks", {
  ...
  expect_s3_class(res, "HapBlockR_epistasis")
...
# =============================================================================
# Section 9: print.HapBlockR_epistasis
# =============================================================================

test_that("print.HapBlockR_epistasis: produces output without error", {
  res <- scan_block_epistasis(
    assoc              = .assoc,
    geno_matrix        = ldx_geno,
    snp_info           = ldx_snp_info,
  ...
  expect_output(print(res), regexp = "HapBlockR Within-Block Epistasis")
})
...
test_that("scan_block_by_block_epistasis: returns empty result when no sig alleles", {
  ...
  expect_s3_class(res, "HapBlockR_block_epistasis")
...
test_that("print.HapBlockR_block_epistasis: produces output without error", {
  res <- scan_block_by_block_epistasis(
    assoc      = .assoc_s,
    haplotypes = .haps_s,
    blues      = .blues_s,
...
test_that(".fit_null_reml: returns named numeric vector matching y names", {
  ...
  resid <- HapBlockR:::.fit_null_reml(y, G, n_pcs = 0L)
...
test_that(".fit_null_reml: n_pcs=1 gives different residuals than n_pcs=0", {
  ...
  r0 <- HapBlockR:::.fit_null_reml(y, G_sym, n_pcs = 0L)
  r1 <- HapBlockR:::.fit_null_reml(y, G_sym, n_pcs = 1L)
