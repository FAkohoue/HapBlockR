...
##   - HapBlockR:::.resolve_overlap()    R reference implementation
##   - HapBlockR::resolve_overlap_cpp()  C++ accelerated implementation
##
...
test_that(".resolve_overlap: non-overlapping blocks pass through unchanged", {
  ...
  result    <- HapBlockR:::.resolve_overlap(blocks_in, adj30, k_rep = 5L)
...
test_that("C++ vs R: non-overlapping input", {
  ...
  r_res  <- HapBlockR:::.resolve_overlap(blocks_in, adj30, k_rep = 5L)
...
test_that(".resolve_overlap: all-left produces two non-overlapping blocks", {
  d      <- .make_allleft_data()
  result <- HapBlockR:::.resolve_overlap(d$blocks, d$adj, k_rep = 5L)
...
test_that("C++ vs R: all-left path produces identical boundaries", {
  d      <- .make_allleft_data()
  r_res  <- HapBlockR:::.resolve_overlap(d$blocks, d$adj, k_rep = 5L)
...
test_that(".resolve_overlap: mixed split produces 2 non-overlapping blocks", {
  d      <- .make_mixed_data()
  result <- HapBlockR:::.resolve_overlap(d$blocks, d$adj, k_rep = 10L)
  expect_equal(nrow(result), 2L)
  expect_true(result[1L, 2L] < result[2L, 1L])
  expect_equal(result[1L, 1L], 1L)
...
test_that("C++ vs R: mixed split produces identical boundaries", {
  d       <- .make_mixed_data()
  r_res   <- HapBlockR:::.resolve_overlap(d$blocks, d$adj, k_rep = 10L)
