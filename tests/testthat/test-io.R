...
test_that("print.HapBlockR_backend outputs type and dimension", {
  G    <- make_tiny()
  info <- data.frame(SNP=colnames(G), CHR="1",
                     POS=seq(1000L,by=2000L,length.out=8L))
  be   <- read_geno(G, format="matrix", snp_info=info)
  expect_output(print(be), "HapBlockR backend")
  expect_output(print(be), "matrix")
  close_backend(be)
})

test_that("summary.HapBlockR_backend shows per-chromosome SNP counts", {
  G    <- make_tiny()
...
test_that("read_geno_bigmemory: reattach from .desc file works", {
  ...
  expect_s3_class(be_bm2, "HapBlockR_backend")
