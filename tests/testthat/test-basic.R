...
test_that("read_geno wraps matrix into valid HapBlockR_backend", {
  data(ldx_geno,     package = "HapBlockR")
  data(ldx_snp_info, package = "HapBlockR")
  be <- read_geno(ldx_geno, format = "matrix", snp_info = ldx_snp_info)
  expect_s3_class(be, "HapBlockR_backend")
...
test_that("read_geno reads numeric CSV from extdata", {
  f <- system.file("extdata","example_genotypes_numeric.csv",package="HapBlockR")
...
test_that("read_geno reads HapMap from extdata", {
  f <- system.file("extdata","example_genotypes.hmp.txt",package="HapBlockR")
...
test_that("read_geno reads VCF from extdata", {
  f <- system.file("extdata","example_genotypes.vcf",package="HapBlockR")
...
test_that("read_geno reads example_gwas.csv from extdata", {
  f <- system.file("extdata","example_gwas.csv",package="HapBlockR")
...
test_that("read_geno reads example_phenotype.csv from extdata", {
  f <- system.file("extdata","example_phenotype.csv",package="HapBlockR")
...
test_that("Big_LD internal: returns valid block data.frame for chr1", {
  ...
  blocks   <- HapBlockR:::Big_LD(ldx_geno[, chr1_idx],
...
test_that("run_Big_LD_all_chr accepts HapBlockR_backend", {
  data(ldx_geno,     package = "HapBlockR")
  data(ldx_snp_info, package = "HapBlockR")
  be     <- read_geno(ldx_geno, format="matrix", snp_info=ldx_snp_info)
...
test_that("example_blues.csv extdata file exists and is readable", {
  f <- system.file("extdata", "example_blues.csv", package = "HapBlockR")
...
test_that("ldx_blues_list loads and has correct structure", {
  data(ldx_blues_list, package = "HapBlockR")
...
test_that("example_blues_env.csv extdata file exists and is readable", {
  f <- system.file("extdata", "example_blues_env.csv", package = "HapBlockR")
...
test_that("run_ldx_pipeline: Path A accepts HapBlockR_backend as geno_source", {
  data(ldx_geno,     package = "HapBlockR")
  data(ldx_snp_info, package = "HapBlockR")
  be <- read_geno(ldx_geno, format = "matrix", snp_info = ldx_snp_info)
...
test_that("run_ldx_pipeline: use_bigmemory=TRUE builds bigmemory backend internally", {
  ...
  # bigmemory >= 1.4.7 names the file "hapblockr_bm.bin";
  # older versions omit the .bin extension ("hapblockr_bm").
  # Accept either form.
  bin_exists  <- any(c("hapblockr_bm.bin", "hapblockr_bm") %in% created_files) ||
    file.exists(file.path(bm_path_fwd, "hapblockr_bm.bin")) ||
    file.exists(file.path(bm_path_fwd, "hapblockr_bm"))
  desc_exists <- "hapblockr_bm.desc"      %in% created_files ||
    file.exists(file.path(bm_path_fwd, "hapblockr_bm.desc"))
  si_exists   <- "hapblockr_bm_snpinfo.rds" %in% created_files ||
    file.exists(file.path(bm_path_fwd, "hapblockr_bm_snpinfo.rds"))
  expect_true(bin_exists,
              label = paste0("hapblockr_bm(.bin) in '", bm_path_norm, "' (found: ",
                             paste(created_files, collapse=", "), ")"))
  expect_true(desc_exists, label = "hapblockr_bm.desc should exist")
  expect_true(si_exists,   label = "hapblockr_bm_snpinfo.rds should exist")
})
...
test_that("run_ldx_pipeline: use_bigmemory reattaches on second call", {
  ...
  # Accept both hapblockr_bm.bin (bigmemory >= 1.4.7) and hapblockr_bm (older).
  ...
  skip_if(!any(c("hapblockr_bm.bin", "hapblockr_bm") %in% created_r),
...
test_that("run_ldx_pipeline: stale imputed cache rebuilt when maf_cut changes", {
  ...
  fp_file <- file.path(bm_path, "hapblockr_bm_imputed_params.rds")
  ...
  expect_true(file.exists(fp_file) ||
                file.exists(file.path(bm_path, "hapblockr_bm_imputed_params.rds")),
...
test_that("run_ldx_pipeline: partial backing files cleaned automatically", {
  ...
  writeLines("stale content", file.path(bm_path, "hapblockr_bm.bin"))
  expect_true(file.exists(file.path(bm_path, "hapblockr_bm.bin")))
  expect_false(file.exists(file.path(bm_path, "hapblockr_bm.desc")))
