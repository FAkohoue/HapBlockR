...
* Remaining note: "checking for future file timestamps ... unable to verify
  current time". This is an environment/connectivity issue (the check
  could not reach an internet time-verification service), not a package
  issue -- there is nothing in the package to change for it, and it does
  not appear consistently across check environments.
* A previous local check also showed a WARNING ("checking for executable
  files") and a NOTE ("checking if this is a source package" -- apparent
  object files/libraries) for two bundled third-party binaries
  (`inst/extdata/AlphaMate.exe`, `inst/extdata/libiomp5md.dll`). Both files
  have been removed from the package; `select_parents_ocs(engine =
  "alphamate")` now requires the caller to supply `alphamate_exe`
  explicitly (see `NEWS.md` and `?select_parents_ocs`, "Providing the
  AlphaMate executable"). No compiled/binary executable of any kind is
  shipped in this source package.
