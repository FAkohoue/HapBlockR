...
compatible_package_version: "0.3.12.9000"
output:
  html_document:
    toc: true
    toc_depth: 4
...
The sections above document each tool and its variants in isolation. This
section instead follows one candidate population through the complete
sequence, genotypes to a certified mating plan, so the relationships between
stages are visible in one place. It mirrors the stage structure of a real
production pipeline (a CIAT Peru rice parent-selection script built on this
package) run here on HapBlockR's own small example dataset
(`ldx_geno`, `ldx_snp_info`, `ldx_blocks`, `ldx_blues`) so every number below
is reproducible. Each stage gives only the call and the decision it feeds;
the tool's own section above, or the vignette named at the end of the stage,
carries the full argument reference and worked comparisons.

Sixteen stages, four phases:

| Phase | Stages | Question |
| :--- | :--- | :--- |
| Evidence | 1-4 | What do we know, and how well do we know it? |
| Shortlist | 5-8 | Which parents are worth keeping? |
| Mate allocation | 9-13 | Which crosses, and how many families from each? |
| Sign-off | 14-16 | Is the plan valid, and is it ready to hand to the programme? |

## Phase 1: Evidence (stages 1-4)

**Stage 1 -- Prepare the targets.** If the phenotype came from a field trial
or genetic evaluation run outside HapBlockR, declare its estimand explicitly
with `prepare_breeding_targets()` (input type, estimation basis, direction,
uncertainty) rather than handing over a raw column of numbers. The example
data stands in for this step with a plain named BLUE vector,
`setNames(ldx_blues$YLD, ldx_blues$id)`; a real programme's `blues.csv` would
go through `prepare_breeding_targets(input_type = "BLUE", ...)` first. See
"Before any recommendation" above and the *programme operations* vignette.

**Stage 2 -- Import genotypes and detect LD blocks.** `read_geno()` reads
VCF/HapMap/GDS/BED/dosage-table input with an explicit multiallelic policy
and immutable sample/variant identity; the example data is already an
in-memory matrix (120 individuals x 230 SNPs across three chromosomes).
`run_Big_LD_all_chr()` (or `Big_LD()` for one chromosome) detects the LD
blocks that every later stage treats as the unit of selection:

```r
blocks <- run_Big_LD_all_chr(be, CLQcut = 0.70)   # or supply ldx_blocks directly
```

See "Genotype import and scalable backends" and "LD calculation and
haplotype-block definition" in the function map below, and the *intro* and
*large-scale* vignettes for file-backed and WGS-scale routes.

**Stage 3 -- Fit the prediction model.** `run_haplotype_prediction()` extracts
haplotype alleles per block, builds the shared GRM, fits marker effects, and
returns whole-genome GEBV together with `gebv_uncertainty` (PEV, reliability,
the `min_reliability` recommendation gate) and `local_gebv` (each
individual's per-block contribution -- the input every shortlist tool below
needs):

```r
pred <- run_haplotype_prediction(
  geno_matrix = ldx_geno, snp_info = ldx_snp_info, blocks = ldx_blocks,
  blues = setNames(ldx_blues$YLD, ldx_blues$id), seed = 1, verbose = FALSE
)
```

**Stage 4 -- Validate with a deployment-matched design.** A model is only as
trustworthy as its validation matches how it will actually be used.
`cv_haplotype_prediction()` reports pooled predictive ability, RMSE, MAE,
bias and calibration slope under `"random"` (interpolation within an
exchangeable population), `"grouped"` (transfer across families/sites -- pass
the same family labels used in stage 7's family-quota selection) or
`"forward"` (prediction into a later, not-yet-observed cycle) designs:

```r
cv <- cv_haplotype_prediction(
  geno_matrix = ldx_geno, snp_info = ldx_snp_info, blocks = ldx_blocks,
  blues = setNames(ldx_blues$YLD, ldx_blues$id), k = 3, validation = "random",
  seed = 1, verbose = FALSE
)
```

See "Genomic prediction, validation and local effects" below and the
*workflow* vignette.

## Phase 2: Shortlist (stages 5-8)

**Stage 5 -- Select the blocks that matter.** `select_top_blocks()` reduces
`pred$block_importance` to the smallest top-ranked set explaining most of the
variance in local GEBV (here, the smallest set explaining 90%); `local_gebv`
restricted to those columns becomes `value_matrix`, the shared input to
truncation, GA, GA+TS and family-quota selection alike:
...
`plot_block_funnel()` shows the ranked-variance elbow before committing to a
cutoff.

**Stage 6 -- Establish the truncation baseline.** `truncation_selection()` on
`pred$gebv` is the reference every alternative tool below is judged against
-- not because it is wrong, but because "how much did the alternative
actually buy us" is only answerable against a stated baseline.

**Stage 7 -- Choose one alternative shortlist tool.** GA parent selection
(joint block coverage, "Coverage-only genetic-algorithm parent selection"),
GA+TS (coverage plus merit, "Joint GA+TS parent selection"), family- or
genetic-cluster-quota selection ("Family or genetic-cluster quota
selection"), or core collection ("Core collection") when diversity itself is
the objective. Pick the one matching the programme's actual constraint (pure
coverage, coverage plus merit, guaranteed family/cluster representation, or
diversity-first) rather than defaulting to whichever ships first in the
package. The *breeding decisions* vignette runs all of these against the
same example data with a forward-simulation comparison of realised genetic
gain.

**Stage 8 -- See the realised gain-vs-diversity tradeoff.**
`select_parents_pareto()` sweeps `select_parents_ga()`'s `coancestry_weight`
across a grid and Pareto-filters the results, so `target_degree` in the mate
-allocation stages below can be set from an observed frontier rather than a
guess. Optional, but recommended whenever OCS's `target_degree` (stage 11)
is being chosen for the first time on a new candidate set.

## Phase 3: Mate allocation (stages 9-13)

**Stage 9 -- Screen candidate crosses for operational feasibility.**
`screen_candidate_crosses()` checks role, fertility, flowering overlap,
heterotic group, quarantine compatibility, forbidden pairs and reciprocal
effects before any cross is ranked on merit -- a cross that cannot physically
be made should never reach the usefulness-criterion ranking below. See
"Operational mating feasibility" and the *programme operations* vignette.

**Stage 10 -- Rank feasible crosses by usefulness criterion.**
`usefulness_criterion()` scores each feasible pair by predicted mid-parent
value plus predicted segregation variance at the target blocks, using
whichever `variance_model` matches the available data (`"block_independent"`
for unphased local GEBV, `"phased"`/`"linked"` for phased haplotypes with or
without linkage, `"simplemating"` when a full multi-locus covariance model is
available). See "Usefulness criterion" above.
...
specific matings deliver that, under an explicit relatedness ceiling set by
`target_degree` (informed by stage 8's frontier). `engine = "alphamate"` and
`engine = "optisel"` both solve the actual OCS problem via genuinely
different solvers; `engine = "simplemating"` is discrete cross
prediction/selection, not true OCS -- see "Engine variants" above before
choosing.
...
quotas, returning every binding constraint alongside the certificate.
`assess_decision_stability()` then compares the certified plan against
threshold, model or population-scenario variants and reports selection
frequency and Jaccard overlap, so "would a slightly different assumption have
chosen a materially different plan" has an actual answer rather than an
assumption. See "Operational mating feasibility" and "Stability, simulation
and validation contracts" below.

**Stage 13 -- Validate against the true optimum.**
`validate_crosses_exact()` solves the same cross-selection problem exactly
(binary integer linear programming) on the same, now-culled candidate list,
and reports the heuristic OCS plan's percentage gap below the true optimum.
Intended as a small-scale sanity check, not a routine replacement for stage
11 -- see "Exact validation" above.
...
`cluster_selection_groups()` across every strategy computed above
(truncation, the chosen alternative, OCS, core collection), show whether the
merit-driven selections concentrate on one or two genetic clusters or
actually spread across the population's real structure.

**Stage 15 -- Validate the result contract.** Every breeder-facing result
from stage 3 onward carries a `hapblockr_result` contract.
`validate_hapblockr_result()` (or `validate()`) checks the schema, immutable
identifiers, input hashes, quality gates and decision table before the result
is treated as a recommendation -- see "Interpreting quality control and
uncertainty" below and complete the "Decision sign-off" template with it.

**Stage 16 -- Export for exchange.** `build_breeding_exchange()` converts a
validated result into BrAPI/MIAPPE-oriented tables (recommendations,
quality gates, uncertainty, exclusions, identifiers, input hashes,
provenance, field mapping) and `write_breeding_exchange()` writes them as a
checksummed CSV bundle -- see "Metadata, exchange and output utilities"
below.

The *full pipeline* vignette runs all sixteen stages above, in this order, on
`ldx_geno`/`ldx_snp_info`/`ldx_blocks`/`ldx_blues`, so every call above can be
copied, run, and inspected directly rather than taken on faith.
