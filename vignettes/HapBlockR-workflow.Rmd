---
title: "HapBlockR Workflow: From Genotype File to Haplotype Features"
...
vignette: >
  %\VignetteIndexEntry{HapBlockR Workflow: From Genotype File to Haplotype Features}
...
This vignette walks through the complete HapBlockR workflow, from reading a
genotype file to obtaining haplotype feature matrices and diversity statistics
-- the mechanical steps a breeding value or a crossing decision ultimately
depends on. The pipeline proceeds in four stages here, plus one more this
vignette sets up but does not cover:

1. **Read** genotype data via `read_geno()` or the all-in-one `run_ldx_pipeline()`
2. **Detect** linkage disequilibrium (LD) blocks chromosome-by-chromosome
   with `run_Big_LD_all_chr()`
...
5. **Decide**: turn a breeding value into local GEBV (Genomic Estimated
   Breeding Value) per block, and local GEBV into an actual set of parents
   -- covered in the *From Local GEBV to a Crossing Decision* vignette,
   which picks up exactly where stage 4 ends.
...
When Genome-Wide Association Study (GWAS)-significant markers are available, `tune_LD_params()` finds the
...
Everything above produces the inputs a breeding decision needs: a genotyped,
block-mapped panel and a haplotype feature matrix or association result. The
*From Local GEBV to a Crossing Decision* vignette picks up from here --
decomposing a breeding value onto these blocks, selecting a founder set with
`select_parents_ga()` against a `truncation_selection()` baseline, and
forward-simulating realised genetic gain for both.
