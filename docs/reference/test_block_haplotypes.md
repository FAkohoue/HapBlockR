...
The null model (\\\sigma_g^2\\, \\\sigma_e^2\\) is fit once per trait
via
[`rrBLUP::mixed.solve()`](https://rdrr.io/pkg/rrBLUP/man/mixed.solve.html)
(REML). Each marker is then tested with a true GLS/EMMAX transform: both
the phenotype and every haplotype-allele column are pre-multiplied by
\\\hat V^{-1/2}\\, where \\\hat V = \hat\sigma_g^2 G + \hat\sigma_e^2
I\\, via the null model's GRM eigendecomposition (computed once per
trait and reused for every marker), and the transformed null design
(intercept + PCs) is projected out via the Frisch-Waugh-Lovell theorem
before the per-allele and per-block tests. This is mathematically
equivalent to a full per-marker GLS fit against \\\hat V\\, not an
approximation. If the null-model fit, its GRM eigendecomposition, or the
transformed null-design inversion fails, the function falls back to a
faster residual-scan approximation (subtracting the estimated fixed
effects and polygenic BLUP, then OLS on what remains) and emits a
warning identifying the affected trait.
...
A named list of class `c("HapBlockR_haplotype_assoc", "list")`:
