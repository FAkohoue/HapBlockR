...
## Phased vs unphased input

Works with both. For unphased blocks (one fixed-width 0/1/2 dosage
string per individual), the "allele" unit is the whole per-individual
dosage string, as in the underlying haplotype table. For phased blocks
(one `"gamete1|gamete2"` pair string per individual, from
[`extract_haplotypes`](https://FAkohoue.github.io/HapBlockR/reference/extract_haplotypes.md)
on phased input), each individual contributes its two constituent gamete
alleles separately to the frequency table and effect decomposition,
giving the correct per-gamete allele frequency (denominator = 2 x n
individuals, missing gametes excluded) rather than treating the
diplotype pair as one label.
