...
- normalize:
  ...
  rescaled across all individuals using `normalize_method`. Set `FALSE`
...
- normalize_method:
  ...
  Character, one of `"minmax"` (default) or `"zscore"`. Only used when
  `normalize = TRUE`.

  `"minmax"`

  :   Linear rescaling to \[0, 1\]: \$\$\text{index}\_i = (S_i -
      S\_{\min}) / (S\_{\max} - S\_{\min})\$\$ where \\S_i\\ is the raw
      sum of block scores. Interpretable as a percentile-like position
      within the panel, but has no direct genetic meaning, and is
      sensitive to the panel's most extreme individual(s) – a single
      outlier compresses everyone else's scores toward 0 or 1. Kept as
      the default for backward compatibility.

  `"zscore"`

  :   Standardisation by the panel's own genetic SD: \$\$\text{index}\_i
      = (S_i - \bar{S}) / \text{sd}(S)\$\$ This is the more standard
      convention in quantitative genetics (selection differentials are
      naturally expressed in genetic SD units) and is more robust to a
      single extreme individual than min-max rescaling, since every
      individual's z-score, not just the min/max pair, moves when the
      panel composition changes. Recommended over `"minmax"` when
      comparing individuals' index values numerically (not just their
      rank) or when panel composition may change between analyses.
