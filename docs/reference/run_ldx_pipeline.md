...
- geno_source:
  ...
  File path or `HapBlockR_backend`. Supported formats: VCF
...
- phase:
  ...
  If `TRUE`, phase with Beagle. Default `FALSE`. When `TRUE`, Beagle is
  given the genotype matrix's ORIGINAL missingness pattern (captured
  before `mean_rounded`/`mode` imputation), so it performs genuine
  HMM-based imputation of missing calls in addition to phasing, rather
  than merely re-phasing already-filled dosages. The result is returned
  separately as `beagle_geno_matrix` (see `Value`); `geno_matrix` always
  remains the `mean_rounded`/`mode`-imputed matrix used for LD block
  detection.
...
- close_gaps_with_snps:

  Logical. If `TRUE`, adjacent LD blocks separated only by unassigned
  SNPs are expanded so that every SNP passing MAF filtering is assigned
  to a block. Default `TRUE`.
...
`hap_matrix_info`, `haplotypes`, `geno_matrix`, `beagle_geno_matrix`,
`snp_info_filtered`, `phased_vcf`, `phased_backend_desc`,
`phase_method`, `n_blocks`, `n_hap_columns`. `beagle_geno_matrix` is an
...
`mean_rounded`/`mode` fallback in `geno_matrix`); it is `NULL` unless
`phase = TRUE`.
