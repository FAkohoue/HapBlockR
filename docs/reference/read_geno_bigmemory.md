...
wraps it in the standard `HapBlockR_backend` interface. Subsequent calls
to
...
- source:
  ...
  Either an `HapBlockR_backend` object (any format), a plain R matrix,
...
- snp_info:
  ...
  and ignored when `source` is a file path or an `HapBlockR_backend` –
...
An `HapBlockR_backend` object with `type = "bigmemory"`. Use
