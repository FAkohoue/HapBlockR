...
A list with `exact_plan` (data frame: the optimal cross selection, a
subset of `data`'s rows), `exact_objective` (the true optimal total
criterion), `n_candidates` (candidate crosses considered after culling),
`status` (lpSolve's solver status; `0` = optimal solution found), and,
if `heuristic_plan` was supplied, `heuristic_objective` and `gap_pct`
(the heuristic plan's percentage shortfall below the exact optimum).
