...
Single Nucleotide Polymorphism (SNP) in a block. Within a high-linkage
disequilibrium (LD) block these strings are nearly 1:1 with true
haplotype classes (Calus et al. 2008) and are sufficient for most
...
- **Heterosis modelling**: identifying heterozygous diplotypes
  unambiguously.
- **Forward-simulating a crossing decision**:
  [`ga_vs_ts_simulation()`](https://FAkohoue.github.io/HapBlockR/reference/ga_vs_ts_simulation.md)
  compares genetic algorithm (GA)-selected against truncation-selected
  founders over simulated generations, and needs to know which alleles
  are in cis to simulate recombination that respects the LD blocks the
  founders were selected on – see the *From Local GEBV to a Crossing
  Decision* vignette for that comparison and Section “Reading a
  pre-phased VCF” below for getting `hap1`/`hap2` into that function.

HapBlockR provides true statistical phasing via Beagle 5.x integration
...
[`run_ldx_pipeline()`](https://FAkohoue.github.io/HapBlockR/reference/run_ldx_pipeline.md)
must therefore point to a VCF or VCF.gz file — Genomic Data Structure
(GDS), PLINK binary genotype (BED), and numeric CSV formats are not
accepted when `phase = TRUE`.
...
| File | Content | Type |
|----|----|----|
| `hapblockr_bm_phased_hap1.bin` | Gamete 1 alleles (individuals × SNPs) | `char` (0/1) |
| `hapblockr_bm_phased_hap2.bin` | Gamete 2 alleles (individuals × SNPs) | `char` (0/1) |
| `hapblockr_bm_phased_dos.bin` | Dosage = hap1 + hap2 (individuals × SNPs) | `char` (0/1/2) |
...
parameters), delete the `hapblockr_bm_phased_*` files from
`bigmemory_path`:

``` r
# From R
files <- list.files("ldx_results/bm_cache", pattern = "hapblockr_bm_phased",
                    full.names = TRUE)
file.remove(files)
...
unlink(list.files("ldx_results/bm_cache",
                   pattern = "hapblockr_bm_phased", full.names = TRUE))
```
