...
This vignette covers linkage disequilibrium (LD) estimation and how
relatedness among individuals distorts it. Getting this choice right
matters beyond block detection itself: every downstream breeding
decision – local GEBV (Genomic Estimated Breeding Value) per block,
genetic algorithm (GA)-based parent selection, family-balance
diagnostics – inherits whatever block boundaries are produced here.
Blocks drawn too broadly because of unmodelled relatedness mean a
parent-selection search (see the *From Local GEBV to a Crossing
Decision* vignette) is optimising over the wrong units. Most breeding
panels – multi-family nurseries, half-sib/full-sib trials, inbred line
panels – are exactly the structured/related case this section is about.

## 1. The kinship problem in LD estimation

The standard squared Pearson correlation between two SNP (Single
Nucleotide Polymorphism) dosage vectors $`g_j`$ and $`g_k`$ is:
...
where $`V = ZZ^\top / (2\sum_j p_j q_j)`$ is the VanRaden (2008) genomic
relationship matrix (GRM) and $`V^{-1/2}`$ is its inverse square root.
Left-multiplying by $`V^{-1/2}`$ removes kinship-induced correlation so
...
chr1](HapBlockR-ld-metrics_files/figure-html/compare-image-1.png)
