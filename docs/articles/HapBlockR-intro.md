# Introduction to HapBlockR

## 1. Overview

`HapBlockR` detects linkage disequilibrium (LD) blocks from genome-wide
...
Everything here builds toward a breeding decision: block detection
defines the units haplotype analysis operates on, haplotype analysis
turns those units into per-block genetic values, and Section 13.5 below
shows the first step of turning that into a ranked list of candidates.
For the full path from there to an actual selected set of parents –
GA-based parent selection against a truncation-selection baseline,
family/diversity checks, and forward-simulated genetic gain – see the
*From Local GEBV to a Crossing Decision* vignette.

------------------------------------------------------------------------

## 2. Why HapBlockR? Relationship to the original Big-LD

HapBlockR is built on the clique-based segmentation algorithm of Kim et
...
– is preserved exactly. HapBlockR extends that foundation to address
...
[`CLQD()`](https://FAkohoue.github.io/HapBlockR/reference/CLQD.md) call,
...
The original Big-LD stops at the block table. HapBlockR provides a
...
size](HapBlockR-intro_files/figure-html/plot-1.png)
...
#> 1    block_1_1000_80183   1     1000  80183     30  FALSE
#> 2   block_1_81064_99022   1    81064  99022     20  FALSE
#> 3 block_1_150109_179371   1   150109 179371     30  FALSE
#> 4    block_2_1000_85240   2     1000  85240     35  FALSE
#> 5  block_2_86236_160549   2    86236 160549     25  FALSE
#> 6 block_2_161515_180473   2   161515 180473     20  FALSE
#> 7    block_3_1000_72892   3     1000  72892     24  FALSE
#> 8  block_3_73704_148576   3    73704 148576     26  FALSE
#> 9 block_3_149647_168376   3   149647 168376     20  FALSE
head(haps[[1]])
#>                           ind001                           ind002 
#> "202222200222200222002022001112" "012101121011011110221210110201" 
#>                           ind003                           ind004 
#> "101112101211101121002011000001" "002000000000000000220200010010" 
#>                           ind005                           ind006 
#> "202222200222200222002022010202" "202222200222200222002022000210"
```
...
#> 1    block_1_1000_80183   1     1000  80183     30   120          116 0.9994398
#> 2   block_1_81064_99022   1    81064  99022     20   120           21 0.8988796
#> 3 block_1_150109_179371   1   150109 179371     30   120          113 0.9990196
#> 4    block_2_1000_85240   2     1000  85240     35   120          106 0.9963585
#> 5  block_2_86236_160549   2    86236 160549     25   120          103 0.9970588
#> 6 block_2_161515_180473   2   161515 180473     20   120           25 0.9155462
#>    Shannon n_eff_alleles freq_dominant sweep_flag phased
#> 1 6.840224       112.500    0.01666667      FALSE  FALSE
#> 2 3.572808         9.207    0.18333333      FALSE  FALSE
#> 3 6.790224       107.463    0.01666667      FALSE  FALSE
#> 4 6.615061        83.721    0.05000000      FALSE  FALSE
#> 5 6.600600        88.889    0.03333333      FALSE  FALSE
#> 6 3.812910        10.860    0.15000000      FALSE  FALSE
```
...
block](HapBlockR-intro_files/figure-html/diversity-plot-1.png)
...
This step maps Genome-Wide Association Study (GWAS) results onto the LD
blocks detected earlier, defining quantitative trait locus (QTL)
regions.
...
#> 1 block_1_150109_179371   1           30             1      rs1070 TraitB
#>   pleiotropic
#> 1       FALSE
subset(qtl, pleiotropic)
#>  [1] block_id                 CHR                      start_bp                
#>  [4] end_bp                   n_snps_block             n_sig_markers           
#>  [7] lead_marker              lead_pos                 lead_p                  
#> [10] lead_log10P              lead_beta                ld_decay_bp             
#> [13] candidate_region_start   candidate_region_end     candidate_region_size_kb
...
rank candidates and inventory their favourable alleles. They stop at a
ranked list – turning that into an actual *set* of parents (GA-optimised
coverage of the important blocks, checked against a truncation-selection
baseline, family-balance diagnostics, and forward-simulated genetic
gain) is covered in the *From Local GEBV to a Crossing Decision*
vignette.
...
When GWAS was run outside HapBlockR, use
...
HapBlockR v0.3.1 adds three epistasis functions that all operate on
