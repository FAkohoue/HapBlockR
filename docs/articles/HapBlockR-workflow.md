# HapBlockR Workflow: From Genotype File to Haplotype Features

## Overview

This vignette walks through the complete HapBlockR workflow, from
reading a genotype file to obtaining haplotype feature matrices and
diversity statistics – the mechanical steps a breeding value or a
crossing decision ultimately depends on. The pipeline proceeds in four
stages here, plus one more this vignette sets up but does not cover:
...
2.  **Detect** linkage disequilibrium (LD) blocks
    chromosome-by-chromosome with
...
5.  **Decide**: turn a breeding value into local GEBV (Genomic Estimated
    Breeding Value) per block, and local GEBV into an actual set of
    parents – covered in the *From Local GEBV to a Crossing Decision*
    vignette, which picks up exactly where stage 4 ends.
...
The `HapBlockR_backend` object stores metadata without loading the full
genotype matrix:
...
When Genome-Wide Association Study (GWAS)-significant markers are
available,
...
haps <- extract_haplotypes(
  geno   = be,           # HapBlockR_backend or plain matrix
...
This section maps GWAS-significant markers onto the haplotype blocks
detected earlier, defining quantitative trait locus (QTL) regions and
scoring them for breeding priority.
...
# Output: class HapBlockR_epistasis
print(epi_within)
...
Everything above produces the inputs a breeding decision needs: a
genotyped, block-mapped panel and a haplotype feature matrix or
association result. The *From Local GEBV to a Crossing Decision*
vignette picks up from here – decomposing a breeding value onto these
blocks, selecting a founder set with
...
baseline, and forward-simulating realised genetic gain for both.
