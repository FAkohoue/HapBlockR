...
external tool invocation. Report it privately to `akohoue.f@gmail.com`
with:
...
HapBlockR invokes external tools only through paths and arguments
supplied or explicitly configured by the user. Users remain responsible
for obtaining those tools from authoritative sources, verifying licences
and checksums, and protecting the input and output directories.
