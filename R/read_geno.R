# -----------------------------------------------------------------------------
# read_geno.R  -  Unified genotype reader for HapBlockR
# -----------------------------------------------------------------------------
#
...
#
# Returns an object of class "HapBlockR_backend" with a unified interface:
...
#' a unified backend object that all \pkg{HapBlockR} functions can consume.
...
#'
#' @return An object of class \code{"HapBlockR_backend"} with elements:
...
.read_numeric <- function(path, sep, na_strings, verbose,
                          chunk_rows = 50000L) {
  if (isTRUE(verbose)) message("[read_geno] Reading numeric dosage (chunked): ", path)
  ...
  if (isTRUE(verbose))
    message("[read_geno]  ", n_snp, " SNPs x ", n_samp, " samples")
...
.read_gds <- function(path, verbose) {
  if (isTRUE(verbose)) message("[read_geno] Opening GDS: ", path)
...
.read_bed <- function(path, verbose) {
  if (isTRUE(verbose)) message("[read_geno] Opening PLINK BED: ", path)
...
#'
#' @param backend An object of class \code{"HapBlockR_backend"} as returned
...
read_chunk <- function(backend, col_idx) {
  ...
  out <- switch(backend$type,
         ...
         gds = {
           ...
           # HapBlockR uses ALT dosage convention (0=hom-REF, 2=hom-ALT).
...
#' file is released.
#'
#' @param backend An \code{"HapBlockR_backend"} object.
#'
#' @return Invisibly \code{NULL}.
#'
#' @export
close_backend <- function(backend) {
  if (!inherits(backend, "HapBlockR_backend")) return(invisible(NULL))
...
#' @param x An \code{HapBlockR_backend} object.
#' @param ... Further arguments passed to or from other methods (unused).
#' @export
print.HapBlockR_backend <- function(x, ...) {
  cat("HapBlockR backend\n")
  cat("  Type      :", x$type, "\n")
...
#' @param object An \code{HapBlockR_backend} object.
#' @param ... Further arguments passed to or from other methods (unused).
#' @export
summary.HapBlockR_backend <- function(object, ...) {
  chrs <- table(.norm_chr(object$snp_info$CHR))
  cat("HapBlockR backend summary\n")
...
#' wraps it in the standard \code{HapBlockR_backend} interface.  Subsequent
...
#'
#' @param source Either an \code{HapBlockR_backend} object (any format), a
...
#'   \code{source} is a file path or an \code{HapBlockR_backend} -- SNP
...
#'
#' @return An \code{HapBlockR_backend} object with \code{type = "bigmemory"}.
...
#' be_bm2 <- read_geno_bigmemory("/data/hapblockr/mydata_bm.desc")
...
read_geno_bigmemory <- function(source,
  ...
  # -- Build from backend ----------------------------------------------------
  if (inherits(source, "HapBlockR_backend")) {
    be      <- source
    si      <- be$snp_info
    n_ind   <- be$n_samples
    n_snps  <- be$n_snps
    ...
    stop("source must be an HapBlockR_backend, a matrix, a genotype file path, ",
