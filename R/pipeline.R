...
.make_imputed_backend <- function(geno_mat, snp_info, sample_ids,
  ...
  .make_backed_backend_impl(geno_mat, snp_info, sample_ids,
                            stem_name = "hapblockr_bm_imputed",
...
.write_cleaned_vcf <- function(geno_mat,
  ...
  writeLines(c(
    "##fileformat=VCFv4.2",
    paste0("##source=HapBlockR_write_cleaned_vcf"),
...
#        hapblockr_bm_phased_hap1  (individuals x SNPs, char, 0/1)
#        hapblockr_bm_phased_hap2  (individuals x SNPs, char, 0/1)
#        hapblockr_bm_phased_dos   (individuals x SNPs, char, 0/1/2)
...
.read_and_cache_phased_vcf <- function(
  ...
  stem_base <- file.path(
    ...
    paste0("hapblockr_bm_phased_", run_tag)
  )
...
#' haplotypes. This is the only supported phasing method in HapBlockR.
#'
...
#'
#' @param geno_source  File path or \code{HapBlockR_backend}. Supported
...
#'   When \code{TRUE}, Beagle is given the genotype matrix's ORIGINAL
#'   missingness pattern (captured before \code{mean_rounded}/\code{mode}
#'   imputation), so it performs genuine HMM-based imputation of missing
#'   calls in addition to phasing, rather than merely re-phasing
#'   already-filled dosages. The result is returned separately as
#'   \code{beagle_geno_matrix} (see \code{Value}); \code{geno_matrix} always
#'   remains the \code{mean_rounded}/\code{mode}-imputed matrix used for LD
#'   block detection.
...
#'   MAF filtering is assigned to a block. Default \code{TRUE}.
...
#'   \code{geno_matrix}, \code{beagle_geno_matrix}, \code{snp_info_filtered},
#'   \code{phased_vcf}, \code{phased_backend_desc}, \code{phase_method},
#'   \code{n_blocks}, \code{n_hap_columns}. \code{beagle_geno_matrix} is an
#'   individuals x SNPs dosage matrix produced by Beagle's own HMM-based
#'   imputation (distinct from, and generally more accurate than, the
#'   \code{mean_rounded}/\code{mode} fallback in \code{geno_matrix}); it is
#'   \code{NULL} unless \code{phase = TRUE}.
#'
#' @examples
#' \donttest{
...
run_ldx_pipeline <- function(
  ...
  if (isTRUE(phase)) {
    if (inherits(geno_source, "HapBlockR_backend"))
      stop(
  ...
  if (inherits(geno_source, "HapBlockR_backend")) {
    ...
    if (!is.character(geno_source) || length(geno_source) != 1L)
      stop("geno_source must be a file path or an HapBlockR_backend.", call. = FALSE)
    ...
    if (isTRUE(use_bigmemory)) {
      ...
      bm_stem      <- file.path(bigmemory_path, "hapblockr_bm")
  ...
  # Snapshot geno_mat's REAL missingness pattern before Step 4.5c overwrites it
  # with mean_rounded/mode-imputed values. When phase = TRUE, this snapshot
  # (not the imputed geno_mat) is fed to Beagle in Step 5b, so Beagle performs
  # genuine HMM-based imputation of missing calls rather than merely re-phasing
  # already-filled data. Only kept when phase = TRUE to avoid the extra memory
  # overhead for callers who never use it.
  geno_mat_preimpute <- if (isTRUE(phase)) geno_mat else NULL
  ...
  blocks <- run_Big_LD_all_chr(
    ...
    close_gaps_with_snps = close_gaps_with_snps,
    gap_k_rep = gap_k_rep,
  ...
  #
  # Beagle is handed geno_mat_preimpute (the pre-imputation snapshot captured
  # above), NOT the mean_rounded/mode-imputed geno_mat used for block
  # detection. Beagle's localized haplotype-cluster HMM performs phasing and
  # missing-genotype imputation jointly and inseparably: when given data with
  # no missing calls, it can only re-phase already-decided dosages, but when
  # given the real missingness pattern it genuinely imputes those calls from
  # local haplotype structure - a statistically stronger method than the
  # column mean/mode fallback used for geno_matrix. The result is surfaced
  # separately as beagle_geno_matrix in the return value; geno_matrix always
  # remains the mean_rounded/mode-imputed matrix for backward compatibility.
  #
  ...
  #     bigmemory_path (hapblockr_bm_phased_hap1, _hap2, _dos)
  ...
  if (isTRUE(phase)) {
    ...
    # by CHR and POS, encoded as 0/0 | 0/1 | 1/1 | ./. GT fields. It is built
    # from geno_mat_preimpute (real missingness intact, pre-Step-4.5c) rather
    # than geno_mat, so Beagle sees genuine missing calls to impute instead of
    # already mean_rounded/mode-filled dosages.
    ...
    # Beagle-imputed dosage matrix (individuals x SNPs), derived from the
    # genuinely-missing-data VCF above. This is Beagle's own HMM-based
    # imputation, distinct from and generally more accurate than the
    # mean_rounded/mode fallback in geno_matrix.
    beagle_geno_matrix <- geno_for_hap$dosage
    if (!is.null(beagle_geno_matrix)) {
      n_na_beagle <- sum(is.na(beagle_geno_matrix))
      if (n_na_beagle > 0L)
        .log(
          "NOTE: beagle_geno_matrix still has ", n_na_beagle,
          " missing values after Beagle imputation (e.g. sites Beagle could ",
          "not resolve). Consider a reference panel via beagle_ref_panel."
        )
    }
  }
  ...
  if (!is.null(hap_bi) && nrow(hap_bi) > 0L) {
    ...
    if (nrow(bad_sing) > 0L)
      stop("[HapBlockR] Singleton-coordinate mismatch in ", nrow(bad_sing),
  ...
  invisible(list(
    ...
    beagle_geno_matrix  = beagle_geno_matrix,
    snp_info_filtered   = snp_info_filtered,
    phased_vcf          = phased_vcf_path,
