...
#'   \code{HapBlockR_backend}. The imputed, MAF-filtered genotype matrix
...
#'
#' @return A named list of class \code{HapBlockR_epistasis}:
#' \describe{
...
scan_block_epistasis <- function(
  ...
  if (!inherits(assoc, "HapBlockR_haplotype_assoc"))
  ...
  is_backend <- inherits(geno_matrix, "HapBlockR_backend")
...
#' @param verbose Logical. Default \code{TRUE}.
#'
#' @return A named list of class \code{HapBlockR_block_epistasis}:
#' \describe{
  ...
  if (is.null(sig_alleles)) {
    ...
    if (!nrow(at_sig)) {
      ...
      return(structure(
        ...
        class = "HapBlockR_block_epistasis"))
    }
...
#' @export
print.HapBlockR_epistasis <- function(x, ...) {
  cat("HapBlockR Within-Block Epistasis Scan\n")
...
#' @export
print.HapBlockR_block_epistasis <- function(x, ...) {
  cat("HapBlockR Between-Block Haplotype Epistasis Scan\n")
