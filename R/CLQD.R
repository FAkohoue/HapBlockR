...
CLQD <- function(
  ...
  # Carry BOTH the original column index (into binvector/SNPbps) and the bp
  # position through every clique in lockstep. An earlier version tracked
  # only bp *values* (cliques_bp) and, at assignment time, recovered column
  # identity via match(best, SNPbps) / setdiff(x, best) -- both operate on
  # bp values, so if two SNP columns share the same bp position (co-located
  # multi-allelic sites, or an indel and a SNP recorded at the same POS --
  # not rare in real VCFs), match() silently returns only the first matching
  # column and setdiff() could strip a *different* SNP that merely shares a
  # bp value with one actually in `best`. Indices are unique identifiers;
  # bp values are not. Tracking indices directly avoids both failure modes.
  cliques_idx <- lapply(cliques, function(x) re_idx[x])
  cliques_bp  <- lapply(cliques, function(x) re_SNPbps[x])
  if (isTRUE(split)) {
    split_out   <- .split_cliques_by_gap(cliques_idx, cliques_bp, clstgap)
    cliques_idx <- split_out$idx
    cliques_bp  <- split_out$bp
  }

  while (any(is.na(binvector)) && length(cliques_bp) > 0L) {
    scores   <- vapply(cliques_bp, clique_score, numeric(1L))
    best_pos <- which.max(scores)
    best_idx <- cliques_idx[[best_pos]]
    binvector[best_idx] <- bin_id
    bin_id <- bin_id + 1L

    new_idx <- list(); new_bp <- list()
    for (k in seq_along(cliques_idx)) {
      rem_idx <- setdiff(cliques_idx[[k]], best_idx)
      if (length(rem_idx) > 1L) {
        rem_bp <- cliques_bp[[k]][match(rem_idx, cliques_idx[[k]])]
        new_idx[[length(new_idx) + 1L]] <- rem_idx
        new_bp[[length(new_bp)  + 1L]]  <- rem_bp
      }
    }
    cliques_idx <- new_idx
    cliques_bp  <- new_bp
...
#' bp positions exceeds \code{gapdist}. Takes and returns the SNP column
#' \emph{indices} alongside their bp positions, sorted/split identically, so
...
#' version that tracked bp values alone and used \code{match()}/\code{setdiff()}
#' against those values (ambiguous whenever a bp value isn't unique).
#'
#' @param cliques_idx List of integer vectors: original column indices per
#'   clique (into \code{binvector}/\code{SNPbps} in \code{CLQD()}).
#' @param cliques_bp List of numeric vectors: bp positions, same length and
#'   order as the corresponding element of \code{cliques_idx}.
#' @param gapdist Numeric. Gap threshold (bp) above which a clique is split.
#'
#' @return Named list with \code{idx} and \code{bp}, each a list of vectors,
#'   parallel to each other, after splitting.
#' @keywords internal
.split_cliques_by_gap <- function(cliques_idx, cliques_bp, gapdist) {
  # Sort each clique's idx/bp pair by bp position (in lockstep), dropping any
  # NA bp positions (and their paired index) exactly as the previous
  # sort(as.numeric(stats::na.omit(x))) did for a single bp-only vector.
  ord_list    <- lapply(cliques_bp, function(bp) order(bp, na.last = NA))
  cliques_idx <- Map(function(idx, ord) idx[ord], cliques_idx, ord_list)
  cliques_bp  <- Map(function(bp,  ord) bp[ord],  cliques_bp,  ord_list)

  final_idx <- list(); final_bp <- list()
  while (length(cliques_bp) > 0L) {
    new_idx <- list(); new_bp <- list()
    for (k in seq_along(cliques_bp)) {
      cl_bp  <- cliques_bp[[k]]
      cl_idx <- cliques_idx[[k]]
      gaps   <- diff(cl_bp)
      if (any(gaps > gapdist)) {
        sp <- which(gaps > gapdist)[1L]
        final_idx[[length(final_idx) + 1L]] <- cl_idx[seq_len(sp)]
        final_bp[[length(final_bp)  + 1L]]  <- cl_bp[seq_len(sp)]
        rest_idx <- cl_idx[(sp + 1L):length(cl_idx)]
        rest_bp  <- cl_bp[(sp + 1L):length(cl_bp)]
        if (length(rest_idx) > 1L) {
          new_idx[[length(new_idx) + 1L]] <- rest_idx
          new_bp[[length(new_bp)  + 1L]]  <- rest_bp
        }
      } else {
        final_idx[[length(final_idx) + 1L]] <- cl_idx
        final_bp[[length(final_bp)  + 1L]]  <- cl_bp
      }
    }
    cliques_idx <- new_idx
    cliques_bp  <- new_bp
  }
  list(idx = final_idx, bp = final_bp)
}
