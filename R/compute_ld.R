...
#' ld_r2  <- HapBlockR:::compute_ld(G, method = "r2")
#' range(ld_r2)
#' }
#'
#' @keywords internal
...
#' in \pkg{HapBlockR} and is 10-50x faster than \code{stats::cor()}.
#'
...
#'     whitening factor for use in \code{appendSGTs}. Missing genotype calls
#'     are mean-imputed (to 0 on the already-centred scale) immediately
#'     before whitening: because the whitening step mixes all individuals
#'     together via the n x n GRM-derived matrix, a single unimputed
#'     \code{NA} would otherwise propagate across the entire SNP column for
#'     every individual once multiplied through, not just the individual
#'     with the missing call.}
#' }
#'
...
prepare_geno <- function(
  ...
  if (isTRUE(verbose)) message("[prepare_geno] Computing VanRaden GRM...")
  kin <- AGHmatrix::Gmatrix(geno, method = "VanRaden")
  dimnames(kin) <- list(ids, ids)

  if (isTRUE(verbose)) message("[prepare_geno] Tuning GRM (bend + rcn)...")
  kin_tuned  <- ASRgenomics::G.tuneup(kin, bend = TRUE, rcn = TRUE)$Gb
  V_inv_sqrt <- get_V_inv_sqrt(kin_tuned, method = kin_method)

  # -- Missing-genotype guard (critical) --------------------------------------
  # V_inv_sqrt %*% geno_centered is a matrix product across ALL individuals
  # (V_inv_sqrt is n x n). R's %*% has no NA-handling: a single NA in column j
  # of geno_centered contaminates (V_inv_sqrt %*% geno_centered)[, j] for
  # EVERY individual, not just the one with the missing call, because every
  # output row is a weighted sum over all n individuals. That NA-column then
  # reaches compute_r2_cpp(), whose per-column mean-impute treats an
  # all-non-finite column as monomorphic and zeroes it -- silently dropping
  # the SNP from clique detection with no warning. Unlike the r2 path (where
  # a missing call only ever affects that one individual before the C++
  # kernel's own per-column mean-impute), the rV2 path must be repaired
  # *before* whitening, since whitening is what mixes individuals together.
  # Mean-impute here (value 0, since geno_centered is already column-centered)
  # so rV2 never silently corrupts a SNP because of a single missing call.
  n_na <- sum(is.na(geno_centered))
  if (n_na > 0L) {
    if (isTRUE(verbose))
      message(sprintf(
        "[prepare_geno] rV2: mean-imputing %d missing genotype call(s) before whitening (imputed to 0 on the centred scale) ...",
        n_na
      ))
    for (j in seq_len(ncol(geno_centered))) {
      na_j <- is.na(geno_centered[, j])
      if (any(na_j)) geno_centered[na_j, j] <- 0
    }
  }
