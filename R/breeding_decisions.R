...
#'   stacking index is rescaled across all individuals using
#'   \code{normalize_method}. Set \code{FALSE} to return raw summed effects
#'   (in phenotype units), which is preferable when comparing candidate sets
#'   across different evaluation sets.
#'
#' @param normalize_method Character, one of \code{"minmax"} (default) or
#'   \code{"zscore"}. Only used when \code{normalize = TRUE}.
#'   \describe{
#'     \item{\code{"minmax"}}{Linear rescaling to [0, 1]:
#'       \deqn{\text{index}_i = (S_i - S_{\min}) / (S_{\max} - S_{\min})}
#'       where \eqn{S_i} is the raw sum of block scores. Interpretable as a
#'       percentile-like position within the panel, but has no direct genetic
#'       meaning, and is sensitive to the panel's most extreme individual(s)
#'       -- a single outlier compresses everyone else's scores toward 0 or 1.
#'       Kept as the default for backward compatibility.}
#'     \item{\code{"zscore"}}{Standardisation by the panel's own genetic SD:
#'       \deqn{\text{index}_i = (S_i - \bar{S}) / \text{sd}(S)}
#'       This is the more standard convention in quantitative genetics
#'       (selection differentials are naturally expressed in genetic SD
#'       units) and is more robust to a single extreme individual than
#'       min-max rescaling, since every individual's z-score, not just the
#'       min/max pair, moves when the panel composition changes. Recommended
#'       over \code{"minmax"} when comparing individuals' index values
#'       numerically (not just their rank) or when panel composition may
...
score_favorable_haplotypes <- function(
    ...
    missing_string   = ".",
    normalize        = TRUE,
    normalize_method = c("minmax", "zscore")
) {
  ...
  for (bn in scored_blocks) {
    ...
    # For phased data hap[ind] = "gamete1|gamete2". The frequency filter must
    # operate on GAMETES (same level as allele_effects$allele), not on full
    # diplotype strings.  For unphased data hap[ind] is already the allele.
    valid <- hap[!grepl(missing_string, hap, fixed = TRUE)]
    if (!length(valid)) next

    is_phased_bn <- any(grepl("|", valid, fixed = TRUE))
    if (is_phased_bn) {
      # Pool g1 and g2 gametes for frequency computation
      parts_valid <- strsplit(valid, "|", fixed = TRUE)
      gametes_valid <- unlist(lapply(parts_valid, function(p)
        p[!grepl(missing_string, p, fixed = TRUE)]))
      freq_tbl <- table(gametes_valid) / length(gametes_valid)
    } else {
    ...
    # Build effect lookup: gamete_string -> effect
    ...
    for (ind in all_ids) {
      ...
      if (is_phased_bn) {
        # Phased: sum effect over both gametes (0/1/2 dosage)
        parts <- strsplit(s, "|", fixed = TRUE)[[1]]
        score_i <- sum(vapply(parts, function(p) {
          if (!grepl(missing_string, p, fixed = TRUE) &&
              p %in% names(eff_map)) eff_map[[p]] else 0
        }, numeric(1L)))
      } else {
        # Unphased: direct string match
  ...
  if (normalize && stats::var(block_sums) > 1e-10) {
    if (normalize_method == "zscore") {
      stack_idx <- (block_sums - mean(block_sums)) / stats::sd(block_sums)
    } else {
...
summarize_parent_haplotypes <- function(
  ...
  for (bn in names(haplotypes)) {
    ...
    # For phased data, compute allele frequencies at the GAMETE level
    # (same level used by build_haplotype_feature_matrix and score_favorable_haplotypes)
    is_phased_bn <- any(grepl("|", valid_all, fixed = TRUE))
    if (is_phased_bn) {
      parts_all  <- strsplit(valid_all, "|", fixed = TRUE)
      gametes_all <- unlist(lapply(parts_all, function(p)
        p[!grepl(missing_string, p, fixed = TRUE)]))
      freq_tbl <- table(gametes_all) / length(gametes_all)
    } else {
    ...
    for (ind in inds) {
      ...
      if (is_phased) {
        parts <- strsplit(s, "|", fixed=TRUE)[[1]]
        # Count copies of each common GAMETE allele
