...
#' }
#'
#' The null model (\eqn{\sigma_g^2}, \eqn{\sigma_e^2}) is fit once per trait via
#' \code{rrBLUP::mixed.solve()} (REML). Each marker is then tested with a true
#' GLS/EMMAX transform: both the phenotype and every haplotype-allele column
#' are pre-multiplied by \eqn{\hat V^{-1/2}}, where
#' \eqn{\hat V = \hat\sigma_g^2 G + \hat\sigma_e^2 I}, via the null model's GRM
#' eigendecomposition (computed once per trait and reused for every marker),
#' and the transformed null design (intercept + PCs) is projected out via the
#' Frisch-Waugh-Lovell theorem before the per-allele and per-block tests. This
#' is mathematically equivalent to a full per-marker GLS fit against
#' \eqn{\hat V}, not an approximation. If the null-model fit, its GRM
#' eigendecomposition, or the transformed null-design inversion fails, the
#' function falls back to a faster residual-scan approximation (subtracting
#' the estimated fixed effects and polygenic BLUP, then OLS on what remains)
#' and emits a warning identifying the affected trait.
#'
...
#'
#' @return A named list of class \code{c("HapBlockR_haplotype_assoc", "list")}:
#'
#' \describe{
...
test_block_haplotypes <- function(
    ...
    if (is.null(eig)) {
      ...
      if (isTRUE(optimize_pcs)) {
        ...
        for (ki in seq_along(k_seq)) {
          ...
          # Deliberately NOT upgraded to the GLS/EMMAX transform used by the
          # main scan below: this loop only ranks candidate n_pcs values
          # (0..optimize_pcs_max) against each other on a 500-allele subsample,
          # so a fast, consistent proxy is sufficient for picking the best k;
          # the actual reported test statistics always come from the full
          # GLS scan run afterwards with the chosen n_pcs.
  ...
  for (tr in traits) {
    ...
    # -- GLS transform (proper EMMAX) -----------------------------------------
    # test_block_haplotypes() is documented as an "EMMAX/GAPIT3 formulation".
    # The correct EMMAX computation transforms BOTH y and every marker column
    # by V^(-1/2), where V = sigma_g^2*K + sigma_e^2*I is the null model's
    # estimated phenotypic covariance, then runs OLS on the transformed data
    # -- mathematically equivalent to a full GLS fit for every marker at the
    # cost of one eigendecomposition, done once per trait.
    #
    # An earlier version of this function instead subtracted the null model's
    # fixed effects AND the polygenic BLUP (y - X*beta - u_hat) and ran plain
    # OLS on what was left. That is a fast, commonly-used approximation, but
    # not the same computation: u_hat is an estimate (not the true random
    # effect), so the remainder is not fully "whitened", and the marker
    # columns were never transformed, so the OLS standard errors did not
    # account for any correlation structure surviving in the residual --
    # weakest exactly in the related/structured populations this machinery
    # exists for. The block below replaces that approximation with the real
    # GLS transform whenever the null model fit succeeds.
    gls_ok <- FALSE
    df_test <- n_ind - 2L   # fallback df; overwritten below when GLS succeeds

    if (!is.null(null_fit)) {
      sigma_g2 <- null_fit$Vu
      sigma_e2 <- null_fit$Ve

      eig_sub <- tryCatch(eigen(G_sub, symmetric = TRUE), error = function(e) NULL)

      if (!is.null(eig_sub) && is.finite(sigma_g2) && is.finite(sigma_e2) &&
          (sigma_g2 + sigma_e2) > 0) {

        U      <- eig_sub$vectors
        lambda <- pmax(eig_sub$values, 0)
        v_diag <- sigma_g2 * lambda + sigma_e2
        v_diag <- pmax(v_diag, 1e-10)          # numerical floor
        w_vec  <- 1 / sqrt(v_diag)

        # V^(-1/2) %*% M, for any n x k matrix M, via the shared eigenbasis:
        # U %*% diag(w) %*% t(U) %*% M == U %*% (w * crossprod(U, M))
        .whiten <- function(M) U %*% (w_vec * crossprod(U, M))

        y_t      <- .whiten(matrix(y, ncol = 1L))
        X_null_t <- .whiten(X_null)

        # Project the null design out of y in the TRANSFORMED (whitened)
        # space -- this is the standard EMMAX shortcut: once whitened, OLS
        # on the transformed variables is algebraically equivalent to GLS
        # on the original ones.
        XtX_null <- crossprod(X_null_t)
        XtX_null_inv <- tryCatch(solve(XtX_null), error = function(e) NULL)

        if (!is.null(XtX_null_inv)) {
          beta_null <- XtX_null_inv %*% crossprod(X_null_t, y_t)
          y_resid_t <- as.numeric(y_t - X_null_t %*% beta_null)
    ...
    if (!gls_ok) {
      ...
      # rather than failing the whole trait.
      if (!is.null(null_fit)) {
        warning("[test_block_haplotypes] GLS transform unavailable for trait '",
                tr, "' (GRM eigendecomposition or null-design inversion failed); ",
                "falling back to the residual-scan approximation.", call. = FALSE)
        y_fixed <- as.numeric(X_null %*% null_fit$beta)
        u_sub   <- null_fit$u[common]; u_sub[is.na(u_sub)] <- 0
        y_resid_t <- y - y_fixed - u_sub
      } else {
        ...
        y_resid_t <- stats::residuals(lm_null)
      }
      # -1 for the marker column being tested; ncol(X_null) already accounts
    ...
    # Mean-impute NA across all columns at once. This must happen BEFORE any
    # matrix-multiply-based whitening below: whitening mixes individuals
    # together (like the rV2 whitening step in prepare_geno()), so an
    # unimputed NA would otherwise contaminate an entire column rather than
    # just the individual with the missing call.
    for (j in seq_len(ncol(X_all))) {
      na_j <- is.na(X_all[, j])
      if (any(na_j)) X_all[na_j, j] <- mean(X_all[, j], na.rm = TRUE)
    }

    # Remove constant columns (evaluated on the RAW dosage matrix -- a column
    # that is monomorphic on the original scale is monomorphic regardless of
    # any later GLS transform).
    ...
    # Meff (simpleM) is about LD/correlation among the tested allele columns
    # themselves, so this must stay on the RAW dosage scale, not the
    # GLS-transformed one.
    meff_cache[[tr]] <- list(
      X_valid      = X_valid,
    ...
    if (gls_ok) {
      # Transform the tested columns the same way y was transformed, then
      # project out the (transformed) null design -- both steps use the SAME
      # eigenbasis and null design established above, so the marker test is
      # a genuine GLS test, not an approximation.
      X_valid_t   <- .whiten(X_valid)
      X_valid_t   <- X_valid_t - X_null_t %*% (XtX_null_inv %*% crossprod(X_null_t, X_valid_t))
      # Frisch-Waugh-Lovell: once y_resid_t and each column of X_valid_t are
      # projected orthogonal to X_null_t (which already contains the
      # transformed intercept), the marker's partial GLS coefficient is the
      # simple regression-through-the-origin of y_resid_t on that column --
      # no further "centering" step is valid or needed here (X_valid_t is
      # orthogonal to X_null_t's column space, not necessarily to the
      # ordinary all-ones vector, since whitening does not preserve that
      # direction unless sigma_g2 = 0).
      y_use       <- y_resid_t
      x_bars_use  <- rep(0, ncol(X_valid_t))
      X_use       <- X_valid_t
    } else {
      y_use      <- y_resid_t
      x_bars_use <- colMeans(X_valid)
      X_use      <- X_valid
    }

    centering_correction <- if (gls_ok) 0 else (colSums(X_use))^2 / n_ind
    XtX_valid <- colSums(X_use^2) - centering_correction
    XtX_valid <- pmax(XtX_valid, 1e-12)

    # Single BLAS call for all allele effects
    Xty    <- as.numeric(crossprod(X_use, y_use))        # x_j' y (already residualised)
    y_bar_r <- if (gls_ok) 0 else mean(y_use)
    Xty_c   <- Xty - n_ind * x_bars_use * y_bar_r   # no-op when gls_ok (both 0)
    beta_all <- Xty_c / XtX_valid                # all betas in one step

    ss_tot   <- sum((y_use - y_bar_r)^2)
    ss_reg   <- beta_all^2 * XtX_valid           # regression SS per allele
    rss_all  <- ss_tot - ss_reg
    rss_all  <- pmax(rss_all, 0)                 # numerical floor
    sigma2   <- rss_all / df_test
    SE_all   <- sqrt(sigma2 / XtX_valid)
    t_all    <- beta_all / SE_all
    p_all    <- 2 * stats::pt(-abs(t_all), df = df_test)
    ...
    for (bn in unique_blocks) {
      ...
      # Use X_use (NOT X_valid): when gls_ok, X_use is the whitened + null-
      # design-projected marker matrix that matches y_use's scale; using the
      # raw dosage matrix here while y_use is on the GLS-transformed scale
      # would silently regress transformed phenotypes on untransformed
      # markers. See the per-allele scan above for the same X_use/y_use pair.
      X_blk   <- X_use[, blk_idx, drop = FALSE]
      ...
      # When gls_ok, X_use/y_use are already whitened AND projected orthogonal
      # to the (transformed) null design -- which includes the intercept --
      # so fitting a second intercept here would be both unnecessary and
      # inconsistent with the per-allele scan's no-intercept convention.
      # In the fallback (non-GLS) path, y_use is only an approximate residual
      # (fixed effects + BLUP subtracted) with no guarantee of exact zero
      # mean, so we keep the intercept there, matching the previous behaviour.
      fit_full <- if (gls_ok) {
        tryCatch(stats::lm(y_r ~ . - 1, data = df_full), error = function(e) NULL)
      } else {
        tryCatch(stats::lm(y_r ~ ., data = df_full), error = function(e) NULL)
      }
      ...
      df_lrt <- ncol(X_blk)
      # Residual df must discount the null design size (1 intercept when no
      # PCs are used, but 1 + n_pcs_used columns otherwise). The previous
      # "n_ind - df_lrt - 1L" silently assumed a single intercept and was too
      # liberal (inflated df_res, anti-conservative p-values) whenever
      # n_pcs_used > 0. When gls_ok, the null design's df is already fully
      # accounted for via the no-intercept fit above, so only ncol(X_null)
      # (not +1) is subtracted; in the fallback path the local fit still
      # estimates its own intercept on top of the approximate residual, so
      # the extra -1 for that intercept is kept.
      df_res <- if (gls_ok) {
        n_ind - ncol(X_null) - df_lrt
      } else {
        n_ind - ncol(X_null) - df_lrt - 1L
      }
...
#' @export
print.HapBlockR_haplotype_assoc <- function(x, ...) {
  cat("HapBlockR Haplotype Association Results\n")
  cat("  Model:        GRM")
  if (x$n_pcs_used > 0L)
...
#' @export
print.HapBlockR_diplotype <- function(x, ...) {
  cat("\nHapBlockR Diplotype Effect Results\n")
  cat("==================================\n")
...
#'       \item \code{Q_df} - degrees of freedom of Q (n_shared_alleles). Each
#'         shared allele contributes its own 1-df, 2-population Cochran's Q;
#'         summing n_shared_alleles independent 1-df tests gives df =
#'         n_shared_alleles (not n_shared_alleles - 1, which would only apply
#'         if all alleles were pooled into a single common effect).
...
compare_block_effects <- function(
  ...
  # -- Input validation --------------------------------------------------------
  if (!inherits(assoc_pop1, "HapBlockR_haplotype_assoc"))
    stop("assoc_pop1 must be of class HapBlockR_haplotype_assoc (output of test_block_haplotypes()).", call. = FALSE)
  if (!inherits(assoc_pop2, "HapBlockR_haplotype_assoc"))
    stop("assoc_pop2 must be of class HapBlockR_haplotype_assoc (output of test_block_haplotypes()).", call. = FALSE)
  ...
  for (tr in traits) {
    ...
    for (bn in all_blocks) {
      ...
      #
      # Each shared allele i gets its OWN pooled estimate b_ivw_i from its OWN
      # 2-population comparison -- i.e. a separate 2-study (k=2), 1-df
      # Cochran's Q per allele. Summing n_v of these independent 1-df
      # contributions gives, under the null, a chi-square with n_v degrees of
      # freedom (sum of n_v iid chi-sq_1 variables = chi-sq_{n_v}). The
      # "n_v - 1" convention only applies to the different scenario of
      # pooling n_v studies into a SINGLE shared effect (one b_ivw for all of
      # them) -- that is not what happens here, so df = n_v, not n_v - 1.
      Q_v  <- sum(w1 * (e1 - b_ivw)^2 + w2 * (e2 - b_ivw)^2)
      Q_df_v <- as.integer(n_v)  # df = n_shared_alleles (sum of n_v independent 1-df tests)
...
#' @export
print.HapBlockR_effect_concordance <- function(x, ...) {
  cat("HapBlockR Cross-Population Effect Concordance\n")
...
#' @examples
#' \donttest{
#' data(ldx_geno, ldx_snp_info, ldx_blocks, ldx_gwas, package = "HapBlockR")
#'
