...
# Seven complementary analysis functions extending the HapBlockR pipeline.
#
...
#' @export
print.HapBlockR_cv <- function(x, ...) {
  cat("HapBlockR Cross-Validation\n")
...
#' returns FST (the full multi-allelic Weir & Cockerham 1984 theta estimator,
#' eq. 2-6: \eqn{\theta = \sum_i a_i / \sum_i (a_i+b_i+c_i)} across alleles
#' \eqn{i}), frequency differences, and a chi-squared test of independence.
#' Useful for detecting blocks under divergent selection or monitoring
#' diversity changes between breeding cycles.
#'
#' For \strong{phased} haplotypes (\code{"gamete1|gamete2"} strings), the true
#' observed-heterozygosity component (\eqn{c_i = \bar{h}_i/2}) is computed
#' from the actual gamete pairs, matching the original Weir & Cockerham (1984)
#' diploid estimator exactly. For \strong{unphased} haplotype strings (which
#' are per-individual diplotype calls, not separable gametes -- see
#' \code{\link{extract_haplotypes}}), the heterozygosity component cannot be
#' observed and is set to 0, which reduces the estimator to its valid
#' "one gene copy per observation" special case rather than approximating a
#' heterozygosity value that cannot be recovered from the data.
#'
...
#'     \item{\code{FST}}{Weir & Cockerham (1984) theta estimator, clamped to
#'       [0,1] (raw estimates can be slightly negative for near-zero true
#'       differentiation; see Weir & Cockerham 1984).}
...
compare_haplotype_populations <- function(
  ...
  rows <- lapply(seq_along(haplotypes), function(i) {
    ...
    ind_names  <- names(hap)
    grp1_calls <- hap[intersect(ind_names, group1)]
    grp2_calls <- hap[intersect(ind_names, group2)]
    grp1_calls <- grp1_calls[!grepl(missing_string, grp1_calls, fixed = TRUE)]
    grp2_calls <- grp2_calls[!grepl(missing_string, grp2_calls, fixed = TRUE)]
    n1 <- length(grp1_calls); n2 <- length(grp2_calls)
    ...
    # Phased data ("gamete1|gamete2" strings) must be split into individual
    # gametes before computing allele frequencies -- otherwise FST and
    # allele frequencies would be computed over DIPLOTYPE combinations
    # instead of true alleles.
    is_phased <- any(grepl("|", c(grp1_calls, grp2_calls), fixed = TRUE))

    if (is_phased) {
      split1 <- strsplit(grp1_calls, "|", fixed = TRUE)
      split2 <- strsplit(grp2_calls, "|", fixed = TRUE)
      g1_hap1 <- vapply(split1, `[`, character(1L), 1L)
      g1_hap2 <- vapply(split1, `[`, character(1L), 2L)
      g2_hap1 <- vapply(split2, `[`, character(1L), 1L)
      g2_hap2 <- vapply(split2, `[`, character(1L), 2L)
      alleles <- union(unique(c(g1_hap1, g1_hap2)), unique(c(g2_hap1, g2_hap2)))
      f1 <- table(factor(c(g1_hap1, g1_hap2), levels = alleles)) / (2 * n1)
      f2 <- table(factor(c(g2_hap1, g2_hap2), levels = alleles)) / (2 * n2)
    } else {
      alleles <- union(unique(grp1_calls), unique(grp2_calls))
      f1 <- table(factor(grp1_calls, levels = alleles)) / n1
      f2 <- table(factor(grp2_calls, levels = alleles)) / n2
    }
    f1 <- as.numeric(f1); f2 <- as.numeric(f2)
    names(f1) <- alleles; names(f2) <- alleles
    ...
    # -- Weir & Cockerham (1984) two-population FST (theta), eq. 2-6 -------
    # Full estimator: theta = sum(a) / sum(a + b + c) across alleles, where
    # a = between-population variance component, b = between-individuals-
    # within-populations, c = between-gametes-within-individuals (the
    # observed-heterozygosity term). r = 2 populations throughout.
    r     <- 2L
    n_tot <- n1 + n2
    n_bar <- n_tot / r
    # n_C = (n_tot - sum(n_i^2)/n_tot) / (r-1); closed form for r=2:
    n_C   <- 2 * n1 * n2 / n_tot
    p_bar <- (n1 * f1 + n2 * f2) / n_tot
    s2    <- (n1 * (f1 - p_bar)^2 + n2 * (f2 - p_bar)^2) / n_bar  # /(n_bar*(r-1)), r-1=1

    if (is_phased) {
      # h_i(allele) = fraction of individuals in population i that are
      # heterozygous carriers of `allele` (exactly one of their two gametes
      # equals `allele`) -- the true Weir-Cockerham observed-heterozygosity
      # term, computable here because phased data gives real gamete pairs.
      het_frac <- function(hapA, hapB, n, allele) {
        if (identical(allele, "other")) {
          is_a <- !(hapA %in% alleles_kept); is_b <- !(hapB %in% alleles_kept)
        } else {
          is_a <- hapA == allele; is_b <- hapB == allele
        }
        sum(xor(is_a, is_b)) / n
      }
      het1 <- vapply(alleles_kept, function(al) het_frac(g1_hap1, g1_hap2, n1, al), numeric(1L))
      het2 <- vapply(alleles_kept, function(al) het_frac(g2_hap1, g2_hap2, n2, al), numeric(1L))
      h_bar <- (n1 * het1 + n2 * het2) / n_tot
    } else {
      # Unphased haplotype strings are per-individual diplotype calls (see
      # extract_haplotypes() phasing caveat): the two chromosomes cannot be
      # separated, so the diploid heterozygosity term is not observable.
      # h_bar = 0 reduces the estimator to its valid "one gene copy per
      # observation" special case rather than guessing at heterozygosity.
      h_bar <- rep(0, length(alleles_kept))
    }

    a_comp <- (n_bar / n_C) *
      (s2 - (1 / (n_bar - 1)) * (p_bar * (1 - p_bar) - s2 / 2 - h_bar / 4))
    b_comp <- (n_bar / (n_bar - 1)) *
      (p_bar * (1 - p_bar) - s2 / 2 - ((2 * n_bar - 1) / (4 * n_bar)) * h_bar)
    c_comp <- h_bar / 2

    denom_sum <- sum(a_comp) + sum(b_comp) + sum(c_comp)
    FST <- if (is.finite(denom_sum) && abs(denom_sum) > 1e-10)
      sum(a_comp) / denom_sum else 0
    FST <- max(0, min(1, FST))
...
run_haplotype_stability <- function(
  ...
  local_by_env <- list()
  # Raw-phenotype mean per environment -- the textbook Finlay-Wilkinson
  # environmental index (mean OBSERVED performance of all genotypes in that
  # environment). Deliberately NOT derived from local_gebv or from the
  # kin.blup() GEBV: both are BLUP/backsolved quantities that are mean-zero
  # by construction within each environment's own fit (a standard property
  # of best-linear-unbiased-prediction random effects, and of the
  # (x - ploidy*p) centring compute_local_gebv() uses throughout the
  # package), so a "grand mean across blocks" computed from either would be
  # ~0 for every environment regardless of real between-environment
  # differences -- making every block's var(I) collapse below the 1e-10
  # threshold and silently produce zero rows. Raw phenotype means carry the
  # real, non-zero-centred environmental level differences this regression
  # needs.
  pheno_env_means <- stats::setNames(rep(NA_real_, length(envs)), envs)
  for (env in envs) {
    ...
    pheno_env_means[env] <- mean(y[common], na.rm = TRUE)
    G_sub <- G[common, common, drop = FALSE]
    fit <- tryCatch(
      rrBLUP::kin.blup(
    ...
    local_gebv <- compute_local_gebv(
      ...
      blocks      = blocks,
      snp_effects = snp_fx,
      # This function's downstream merge (below) assumes local_gebv's
      # columns correspond 1:1 with rows of `blocks` (block_id is
      # reconstructed directly from blocks' own CHR/start.bp/end.bp).
      # complete_decomposition = TRUE (the compute_local_gebv() default as
      # of 0.3.4.9000) would add extra singleton pseudo-block columns for
      # SNPs outside every block window, which have no corresponding row in
      # `blocks` and would break that merge. Keep this call's behaviour
      # exactly as it was before complete_decomposition existed.
  ...
  # Environmental index: raw-phenotype mean per environment (see note above
  # local_by_env's initialisation), restricted to the environments that
  # actually made it into local_by_env (some may have been skipped for
  # insufficient common individuals or a failed kin.blup() fit).
  env_means <- pheno_env_means[names(local_by_env)]
  ...
  rows <- lapply(block_ids, function(bid) {
    ...
    # Environmental index I_j = mean OBSERVED phenotype in env j (Finlay &
    # Wilkinson 1963's original definition) -- NOT a mean of GEBV/local_gebv,
    # which is ~0 in every environment by construction (see note above).
    I <- env_means
...
#'   Default \code{"."}.
#'
#' @section Phased vs unphased input:
#' Works with both. For unphased blocks (one fixed-width 0/1/2 dosage string
#' per individual), the "allele" unit is the whole per-individual dosage
#' string, as in the underlying haplotype table. For phased blocks (one
#' \code{"gamete1|gamete2"} pair string per individual, from
#' \code{\link{extract_haplotypes}} on phased input), each individual
#' contributes its two constituent gamete alleles separately to the
#' frequency table and effect decomposition, giving the correct
#' per-gamete allele frequency (denominator = 2 x n individuals, missing
#' gametes excluded) rather than treating the diplotype pair as one label.
#'
...
decompose_block_effects <- function(
  ...
  rows <- lapply(seq_along(haplotypes), function(i) {
    ...
    #
    # extract_haplotypes() emits two different per-individual string shapes
    # (see its block_info$phased column / extract_chr_haplotypes_phased_cpp()
    # in src/ld_core.cpp): unphased blocks get ONE fixed-width dosage string
    # per individual (one 0/1/2 digit per SNP); phased blocks get a
    # "gamete1|gamete2" PAIR string per individual (one 0/1 digit per SNP,
    # per gamete). Treating a phased pair string as a single "allele" (the
    # original behaviour here) made every phased allele string longer than
    # the block's SNP count by construction, so the digit-count check further
    # below silently returned NA for every allele and NULLed the whole block
    # out with no warning. Phased pairs are now split into their two
    # constituent gamete alleles before frequency tabulation, which also
    # gives the biologically correct allele-frequency denominator (2 gametes
    # per individual) instead of one diplotype-string label per individual.
    is_phased <- if (!is.null(blk) && "phased" %in% names(blk)) {
      isTRUE(blk$phased[1])
    } else {
      any(grepl("|", hap, fixed = TRUE))
    }

    if (is_phased) {
      valid_pairs <- hap[!grepl(missing_string, hap, fixed = TRUE)]
      if (!length(valid_pairs)) return(NULL)
      valid <- unlist(strsplit(valid_pairs, "|", fixed = TRUE), use.names = FALSE)
      valid <- valid[nzchar(valid) & !grepl(missing_string, valid, fixed = TRUE)]
    } else {
      valid <- hap[!grepl(missing_string, hap, fixed = TRUE)]
    }
...
#' @examples
#' \donttest{
#' data(ldx_geno, ldx_snp_info, package = "HapBlockR")
...
scan_diversity_windows <- function(
  ...
  for (ch in chrs) {
    ...
    for (ws in starts) {
      ...
      He   <- 1 - sum(freq^2)
      He_c <- (ni / (ni - 1L)) * He
      # Shannon entropy in bits (log base 2) -- see compute_haplotype_diversity()
