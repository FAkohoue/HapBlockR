...
#' @param CLQcut,clstgap,leng,subSegmSize,MAFcut,appendrare,close_gaps_with_snps,gap_k_rep,checkLargest,CLQmode,kin_method,split,digits,seed,verbose
...
run_Big_LD_all_chr <- function(
  ...
  # -- Accept either a plain matrix+snp_info OR an HapBlockR_backend ----------
  if (inherits(geno_matrix, "HapBlockR_backend")) {
    backend  <- geno_matrix
  ...
  for (chr_i in chromosomes) {
    if (isTRUE(verbose)) message("[run_Big_LD_all_chr] Processing ", chr_i, " ...")
    ...
    if (ncol(geno_chr) < as.integer(min_snps_chr)) {
      if (isTRUE(verbose)) message("  Skipping ", chr_i, " - fewer than ", min_snps_chr, " SNPs.")
      next
    }
    ...
    blk <- tryCatch(
      Big_LD(geno = geno_chr, SNPinfo = info_chr,
             ...
             close_gaps_with_snps = close_gaps_with_snps, gap_k_rep = gap_k_rep,
      ...
      error = function(e) {
        if (isTRUE(verbose))
          message("  Error in Big_LD for ", chr_i, ": ", conditionMessage(e))
        NULL
      }
    )
...
.build_blocks_chr <- function(geno_chr, info_chr, params) {
  Big_LD(
    ...
    singleton_as_block = params$singleton_as_block,
    close_gaps_with_snps = params$close_gaps_with_snps,
    gap_k_rep = params$gap_k_rep,
