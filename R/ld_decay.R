...
#' \eqn{d} (in Mb) via \eqn{C = 4 N_e r d}. HapBlockR fits this per chromosome with C as the
...
#' }
#'
#' @param geno \code{HapBlockR_backend} (from \code{\link{read_geno}} or
...
#'
#' @return A named list of class \code{HapBlockR_decay}:
#' \describe{
...
compute_ld_decay <- function(
  ...
  # -- Open backend ---------------------------------------------------------
  own_backend <- FALSE
  if (inherits(geno, "HapBlockR_backend")) {
    be <- geno
    ...
    stop("geno must be an HapBlockR_backend, matrix, or file path.", call. = FALSE)
  }
  ...
  .prep_col <- function(x) {
    ...
    if (!is.null(V_inv_sqrt)) {
      x <- as.numeric(V_inv_sqrt %*% x)
    }
  ...
  if (use_param && length(chrs) >= 2L) {
    ...
    unlinked_vals <- unlinked_vals[
      is.finite(unlinked_vals) &
        !is.na(unlinked_vals) &
        unlinked_vals >= 0 &
        unlinked_vals <= 1
    ]
    ...
    if (length(unlinked_vals) > 0L) {
      critical_r2_param <- stats::quantile(
        unlinked_vals,
        pctile / 100,
        na.rm = TRUE
      )
    } else {
      critical_r2_param <- NULL
    }
  ...
  critical_r2 <- if (
    use_param &&
    !is.null(critical_r2_param) &&
    is.finite(critical_r2_param)
  ) {
    critical_r2_param
  } else if (use_fixed) {
    fixed_val
  ...
  for (cb in chrs) {
    ...
    if (fit_model %in% c("loess", "both") && nrow(curve) >= 5L) {
      tryCatch({
        ...
        curve$r2_loess <- pmax(
          0,
          stats::predict(lo, curve$dist_bp)
        )

        # Prevent LOESS from starting below the observed first-bin LD mean.
        # This avoids visual artefacts where the fitted curve suppresses
        # the short-distance LD peak.
        curve$r2_loess[1L] <- max(
          curve$r2_loess[1L],
          curve$r2_mean[1L],
          na.rm = TRUE
        )

        # Enforce non-increasing LD decay from short to long distance.
        curve$r2_loess <- cummin(curve$r2_loess)

      }, error = function(e) {
        warning(
          "[ld_decay] LOESS fit failed for chr ", cb,
          ": ", conditionMessage(e),
          call. = FALSE
        )
      })
    ...
    ##############################################################################
    # Hill-Weir nonlinear curve for plotting / comparison only
    ##############################################################################
    ...
    if (fit_model %in% c("nonlinear", "both") && nrow(df_chr) >= 10L) {
      tryCatch({
        ...
        curve$r2_hw <- pmax(
          0,
          .hw_expect(C_hat, n_ind, curve$dist_bp / 1e6)
        )

      }, error = function(e) {
        warning(
          "[ld_decay] Hill-Weir nonlinear fit failed for chr ", cb,
          ": ", conditionMessage(e),
    ...
    if (!is.null(critical_r2)) {
      ...
      valid_idx <- which(
        is.finite(smooth) &
          !is.na(smooth) &
          is.finite(curve$dist_bp)
      )

      if (length(valid_idx) < 3L) {
        ...
        warning(
          "[ld_decay] chr ", cb,
          ": fewer than three valid decay points. decay_dist is censored.",
          call. = FALSE
        )

      } else {

        dist_valid <- curve$dist_bp[valid_idx]
        smooth_valid <- smooth[valid_idx]

        # If LD is already below the threshold in the first distance bin,
        # the decay distance is the first observed bin, not max_dist.
        if (smooth_valid[1L] <= critical_r2) {

          # The curve is already below the threshold at the first observed bin.
          # This is a left-censored decay distance: true crossing occurred before
          # the first bin midpoint. Return the first bin as an upper-bound estimate.
          censored <- TRUE
          d_bp <- dist_valid[1L]

          warning(
            "[ld_decay] chr ", cb,
            ": LD is already below threshold ",
            round(critical_r2, 4),
            " at the first distance bin. decay_dist is left-censored.",
            call. = FALSE
          )

        } else {

          below <- which(smooth_valid <= critical_r2)

          if (length(below) == 0L) {

            # LD remains above the threshold across all observed bins.
            censored <- TRUE
            d_bp <- max(dist_valid, na.rm = TRUE)

            warning(
              "[ld_decay] chr ", cb,
              ": LD does not decay below threshold ",
              round(critical_r2, 4),
              " within max_dist = ",
              format(max_dist, big.mark = ","),
              " bp. decay_dist is censored.",
              call. = FALSE
            )
            ...
            if (first_below == 1L) {
              ...
              x1 <- dist_valid[first_below - 1L]
              y1 <- smooth_valid[first_below - 1L]

              x2 <- dist_valid[first_below]
              y2 <- smooth_valid[first_below]

              if (
                !is.finite(y1) ||
                !is.finite(y2) ||
                abs(y2 - y1) < .Machine$double.eps
              ) {
                d_bp <- x2
              } else {
  ...
  structure(list(
    ...
    pctile             = pctile,
    call               = cl
  ), class = c("HapBlockR_decay", "list"))
}


#' @export
print.HapBlockR_decay <- function(x, ...) {
  cat("HapBlockR LD Decay Analysis\n")
  cat("  Method      :", x$method, "\n")
...
#'
#' @param x \code{HapBlockR_decay} object.
...
plot_ld_decay <- function(x, plot_points = FALSE, plot_threshold = TRUE,
                          ...
                          facet = FALSE, ...) {
  if (!inherits(x, "HapBlockR_decay"))
    stop("x must be an HapBlockR_decay object.", call. = FALSE)
